...
#DesignerProperty: Key: LazyValidation, DisplayName: LazyValidation, FieldType: Boolean, DefaultValue: false , Description: Set lazy-validation
...
Sub Class_Globals
	...
	Public bLazyValidation As Boolean = false
...
Public Sub DesignerCreateView (Target As BANanoElement, props As Map)
	...
	If props <> Null Then
		...
		AddAttr(Attributes, bLazyValidation, "b", "lazy-validation")
...
'set lazy-validation
Sub SetLazyValidation(varLazyValidation As Boolean)
	bLazyValidation = varLazyValidation
	mElement.SetAttr("lazy-validation", bLazyValidation)
End Sub
...
'set on submit event
Sub SetOnSubmit
	Dim mName As String = $"${mEventName}_submit"$
	mName = mName.tolowercase
	If SubExists(mCallBack, mName) = False Then Return
	SetAttr("v-on:submit", mName)
