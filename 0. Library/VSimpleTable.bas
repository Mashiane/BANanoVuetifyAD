2020-07-08 02:39:24 B4J=true
Group=Default Group
ModulesStructureVersion=1
Type=Class
Version=8.3
@EndOfDesignText@
'Custom BANano View class: VSimpleTable
#IgnoreWarnings:12
...
Private mTagName As String = "v-simple-table" 
...
Public Sub Initialize (CallBack As Object, Name As String, EventName As String) As VSimpleTable 
mName = Name 
...
'set dark
Sub SetDark(varDark As Boolean) As VSimpleTable
bDark = varDark
SetAttr("dark", bDark)
Return Me
End Sub
...
'set dense
Sub SetDense(varDense As Boolean) As VSimpleTable
bDense = varDense
...
'set disabled
Sub SetDisabled(varDisabled As String) As VSimpleTable
...
'set fixed-header
Sub SetFixedHeader(varFixedHeader As Boolean) As VSimpleTable
...
'set height
Sub SetHeight(varHeight As String) As VSimpleTable
sHeight = varHeight
...
'set key
Sub SetKey(varKey As String) As VSimpleTable
sKey = varKey
SetAttr("key", sKey)
Return Me
End Sub
...
'set light
Sub SetLight(varLight As Boolean) As VSimpleTable
bLight = varLight
...
'set parent-id
Sub SetParentId(varParentId As String) As VSimpleTable
...
'set readonly
Sub SetReadonly(varReadonly As String) As VSimpleTable
...
'set ref
Sub SetRef(varRef As String) As VSimpleTable
sRef = varRef
SetAttr("ref", sRef)
Return Me
End Sub
...
'set required
Sub SetRequired(varRequired As String) As VSimpleTable
...
'set v-bind:class
Sub SetVBindClass(varVBindClass As String) As VSimpleTable
...
'set v-bind:style
Sub SetVBindStyle(varVBindStyle As String) As VSimpleTable
...
'set v-cloak
Sub SetVCloak(varVCloak As Boolean) As VSimpleTable
bVCloak = varVCloak
...
'set v-else
Sub SetVElse(varVElse As String) As VSimpleTable
sVElse = varVElse
...
'set v-else-if
Sub SetVElseIf(varVElseIf As String) As VSimpleTable
sVElseIf = varVElseIf
...
'set v-for
Sub SetVFor(varVFor As String) As VSimpleTable
sVFor = varVFor
SetAttr("v-for", sVFor)
Return Me
End Sub
...
'set v-html
Sub SetVHtml(varVHtml As String) As VSimpleTable
sVHtml = varVHtml
...
'set v-if
Sub SetVIf(varVIf As String) As VSimpleTable
sVIf = varVIf
SetAttr("v-if", sVIf)
Return Me
End Sub
...
'set v-model
Sub SetVModel(varVModel As String) As VSimpleTable
sVModel = varVModel
...
'set v-once
Sub SetVOnce(varVOnce As Boolean) As VSimpleTable
bVOnce = varVOnce
...
'set v-pre
Sub SetVPre(varVPre As Boolean) As VSimpleTable
bVPre = varVPre
SetAttr("v-pre", bVPre)
Return Me
End Sub
...
'set v-show
Sub SetVShow(varVShow As String) As VSimpleTable
sVShow = varVShow
...
'set v-text
Sub SetVText(varVText As String) As VSimpleTable
sVText = varVText
...
'set border-color
Sub SetBorderColor(varBorderColor As String) As VSimpleTable
...
'set border-style
Sub SetBorderStyle(varBorderStyle As String) As VSimpleTable
...
'set border-width
Sub SetBorderWidth(varBorderWidth As String) As VSimpleTable
...
'set border-radius
Sub SetBorderRadius(varBorderRadius As String) As VSimpleTable
...
'set margin-top
Sub SetMarginTop(varMarginTop As String) As VSimpleTable
...
'set margin-right
Sub SetMarginRight(varMarginRight As String) As VSimpleTable
...
'set margin-bottom
Sub SetMarginBottom(varMarginBottom As String) As VSimpleTable
...
'set margin-left
Sub SetMarginLeft(varMarginLeft As String) As VSimpleTable
...
'set padding-top
Sub SetPaddingTop(varPaddingTop As String) As VSimpleTable
...
'set padding-right
Sub SetPaddingRight(varPaddingRight As String) As VSimpleTable
...
'set padding-bottom
Sub SetPaddingBottom(varPaddingBottom As String) As VSimpleTable
...
'set padding-left
Sub SetPaddingLeft(varPaddingLeft As String) As VSimpleTable
...
Sub AddComponent(child As String) As VSimpleTable
...
Sub SetID(varText As String) As VSimpleTable
	mname = varText
	Return Me
End Sub
...
public Sub AddToParent(targetID As String) As VSimpleTable
...
Sub AddToApp(vap As VueApp) As VSimpleTable
	appLink = vap
	data = vap.data	
...
'update the state
Sub SetData(prop as string, value as object) As VSimpleTable
	data.put(prop, value)
	Return Me
End Sub
...
'set an event
Sub SetVOn(event As String) As VSimpleTable
...
Sub SetCaption(varText As String) As VSimpleTable
	If mElement <> Null Then
...
private Sub AddAttr(varName As String, actProp As String) As VSimpleTable
...
public Sub SetClasses(Classes As String) As VSimpleTable
	If mElement <> Null Then
...
public Sub SetStyle(Style As String) As VSimpleTable
	If mElement <> Null Then
...
Sub AddClass(classNames As List) As VSimpleTable
...
Sub SetStyles(m As Map) As VSimpleTable
...
Sub SetAttrs(props As Map) As VSimpleTable
...
'set an attribute
Sub SetAttr(prop As String, value As String) As VSimpleTable
...
'bind an attribute
Sub SetVBind(prop As String, value As String) As VSimpleTable
...
'set a single style
Sub SetStyleSingle(prop As String, value As String) As VSimpleTable
	If BANano.IsUndefined(prop) or BANano.IsNull(prop) Then prop = ""
	If BANano.IsUndefined(value) or BANano.IsNull(value) Then value = ""
	if prop = "" then return me
...
Sub Build(props As Map, styleProps As Map, classNames As List, loose As List) As VSimpleTable
	If loose <> Null Then
...
'bind classes
Sub SetVClass(classObj as string) As VSimpleTable
	SetVBind("class", classObj)
	Return Me
End Sub

'bind styles
Sub SetVStyle(styleObj as string) As VSimpleTable
	SetVBind("style", styleObj)
	Return Me
End Sub

'set color
Sub SetColor1(varColor As String) As VSimpleTable
...
'set color intensity
Sub SetColorIntensity(varColor As String, varIntensity As String) As VSimpleTable
...
'set text color
Sub SetTextColor1(varColor As String) As VSimpleTable
...
Sub SetTextColorIntensity(varColor As String, varIntensity As String) As VSimpleTable
	Dim sColor As String = $"${varColor}--text"$
...
'set direct method
Sub SetMethod(Module As Object,methodName As String) As VSimpleTable
	methodName = methodName.ToLowerCase
...
'toggle
Sub Toggle As VSimpleTable
	If sVShow = "" Then
		Log($"VSimpleTable.Toggle - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
'hide
Sub Hide As VSimpleTable
	If sVShow = "" Then
		Log($"VSimpleTable.Hide - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
'show
Sub Show As VSimpleTable
	If sVShow = "" Then
		Log($"VSimpleTable.Show - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
Sub SetClassOnOff(clsName as string, clsValue As Boolean) As VSimpleTable
	if svBindClass = "" then
		Log($"VSimpleTable.VBindClass - the v-bind:class for ${mName} has not been set!"$)
		Return Me
	end if
...
'set style 
Sub SetStyleOnOff(styleName as string, styleValue As Boolean) As VSimpleTable
	if svBindStyle = "" then
		Log($"VSimpleTable.VBindCStyle - the v-bind:style for ${mName} has not been set!"$)
		Return Me
	end if
...
'required
'Sub SetRequiredOnOff(b As Boolean) As VSimpleTable
'	If sRequired = "" Then
'		Log($"VSimpleTable.Required - the required for ${mName} has not been set!"$)
'		Return Me
'	End If
...
'read only
'Sub SetReadOnlyOnOff(b As Boolean) As VSimpleTable
'	If sReadonly = "" Then
'		Log($"VSimpleTable.ReadOnly - the readonly for ${mName} has not been set!"$)
'		Return Me
'	End If
...
'disabled
'Sub SetDisabledOnOff(b As Boolean) As VSimpleTable
'	If sDisabled = "" Then
'		Log($"VSimpleTable.Disabled - the disabled for ${mName} has not been set!"$)
'		Return Me
'	End If
