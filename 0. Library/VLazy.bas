2020-07-08 02:38:39 B4J=true
Group=Default Group
ModulesStructureVersion=1
Type=Class
Version=8.3
@EndOfDesignText@
'Custom BANano View class: VLazy
#IgnoreWarnings:12
...
Private mTagName As String = "v-lazy" 
...
Public Sub Initialize (CallBack As Object, Name As String, EventName As String) As VLazy 
mName = Name 
...
'set disabled
Sub SetDisabled(varDisabled As String) As VLazy
...
'set options
Sub SetOptions(varOptions As String) As VLazy
sOptions = varOptions
...
'set parent-id
Sub SetParentId(varParentId As String) As VLazy
...
'set readonly
Sub SetReadonly(varReadonly As String) As VLazy
...
'set ref
Sub SetRef(varRef As String) As VLazy
sRef = varRef
SetAttr("ref", sRef)
Return Me
End Sub
...
'set required
Sub SetRequired(varRequired As String) As VLazy
...
'set tag
Sub SetTag(varTag As String) As VLazy
sTag = varTag
SetAttr("tag", sTag)
Return Me
End Sub
...
'set transition
Sub SetTransition(varTransition As String) As VLazy
...
'set v-bind:class
Sub SetVBindClass(varVBindClass As String) As VLazy
...
'set v-bind:style
Sub SetVBindStyle(varVBindStyle As String) As VLazy
...
'set v-cloak
Sub SetVCloak(varVCloak As Boolean) As VLazy
bVCloak = varVCloak
...
'set v-else
Sub SetVElse(varVElse As String) As VLazy
sVElse = varVElse
...
'set v-html
Sub SetVHtml(varVHtml As String) As VLazy
sVHtml = varVHtml
...
'set v-if
Sub SetVIf(varVIf As String) As VLazy
sVIf = varVIf
SetAttr("v-if", sVIf)
Return Me
End Sub
...
'set v-model
Sub SetVModel(varVModel As String) As VLazy
sVModel = varVModel
...
'set v-once
Sub SetVOnce(varVOnce As Boolean) As VLazy
bVOnce = varVOnce
...
'set v-pre
Sub SetVPre(varVPre As Boolean) As VLazy
bVPre = varVPre
SetAttr("v-pre", bVPre)
Return Me
End Sub
...
'set v-show
Sub SetVShow(varVShow As String) As VLazy
sVShow = varVShow
...
'set v-text
Sub SetVText(varVText As String) As VLazy
sVText = varVText
...
'set border-color
Sub SetBorderColor(varBorderColor As String) As VLazy
...
'set border-style
Sub SetBorderStyle(varBorderStyle As String) As VLazy
...
'set border-width
Sub SetBorderWidth(varBorderWidth As String) As VLazy
...
'set border-radius
Sub SetBorderRadius(varBorderRadius As String) As VLazy
...
'set margin-top
Sub SetMarginTop(varMarginTop As String) As VLazy
...
'set margin-right
Sub SetMarginRight(varMarginRight As String) As VLazy
...
'set margin-bottom
Sub SetMarginBottom(varMarginBottom As String) As VLazy
...
'set margin-left
Sub SetMarginLeft(varMarginLeft As String) As VLazy
...
'set padding-top
Sub SetPaddingTop(varPaddingTop As String) As VLazy
...
'set padding-right
Sub SetPaddingRight(varPaddingRight As String) As VLazy
...
'set padding-bottom
Sub SetPaddingBottom(varPaddingBottom As String) As VLazy
...
'set padding-left
Sub SetPaddingLeft(varPaddingLeft As String) As VLazy
...
Sub AddComponent(child As String) As VLazy
...
Sub SetID(varText As String) As VLazy
	mname = varText
	Return Me
End Sub
...
public Sub AddToParent(targetID As String) As VLazy
...
Sub AddToApp(vap As VueApp) As VLazy
	appLink = vap
	data = vap.data	
...
'update the state
Sub SetData(prop as string, value as object) As VLazy
	data.put(prop, value)
	Return Me
End Sub
...
'set an event
Sub SetVOn(event As String) As VLazy
...
Sub SetCaption(varText As String) As VLazy
	If mElement <> Null Then
...
public Sub SetClasses(Classes As String) As VLazy
	If mElement <> Null Then
...
public Sub SetStyle(Style As String) As VLazy
	If mElement <> Null Then
...
Sub AddClass(classNames As List) As VLazy
...
Sub SetStyles(m As Map) As VLazy
...
Sub SetAttrs(props As Map) As VLazy
...
'set an attribute
Sub SetAttr(prop As String, value As String) As VLazy
...
'bind an attribute
Sub SetVBind(prop As String, value As String) As VLazy
...
'set a single style
Sub SetStyleSingle(prop As String, value As String) As VLazy
	If BANano.IsUndefined(prop) or BANano.IsNull(prop) Then prop = ""
	If BANano.IsUndefined(value) or BANano.IsNull(value) Then value = ""
	if prop = "" then return me
...
Sub Build(props As Map, styleProps As Map, classNames As List, loose As List) As VLazy
	If loose <> Null Then
...
'bind classes
Sub SetVClass(classObj as string) As VLazy
	SetVBind("class", classObj)
	Return Me
End Sub

'bind styles
Sub SetVStyle(styleObj as string) As VLazy
	SetVBind("style", styleObj)
	Return Me
End Sub

'set color
Sub SetColor1(varColor As String) As VLazy
...
'set color intensity
Sub SetColorIntensity(varColor As String, varIntensity As String) As VLazy
...
'set text color
Sub SetTextColor1(varColor As String) As VLazy
...
Sub SetTextColorIntensity(varColor As String, varIntensity As String) As VLazy
	Dim sColor As String = $"${varColor}--text"$
...
'set direct method
Sub SetMethod(Module As Object,methodName As String) As VLazy
	methodName = methodName.ToLowerCase
...
Sub Toggle As VLazy
	If sVShow = "" Then
		Log($"VLazy.Toggle - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
Sub Hide As VLazy
	If sVShow = "" Then
		Log($"VLazy.Hide - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
Sub Show As VLazy
	If sVShow = "" Then
		Log($"VLazy.Show - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
Sub SetClassOnOff(clsName as string, clsValue As Boolean) As VLazy
	if svBindClass = "" then
		Log($"VLazy.VBindClass - the v-bind:class for ${mName} has not been set!"$)
		Return Me
	end if
...
'set style 
Sub SetStyleOnOff(styleName as string, styleValue As Boolean) As VLazy
	if svBindStyle = "" then
		Log($"VLazy.VBindCStyle - the v-bind:style for ${mName} has not been set!"$)
		Return Me
	end if
...
'required
'Sub SetRequiredOnOff(b As Boolean) As VLazy
'	If sRequired = "" Then
'		Log($"VLazy.Required - the required for ${mName} has not been set!"$)
'		Return Me
'	End If
...
'read only
'Sub SetReadOnlyOnOff(b As Boolean) As VLazy
'	If sReadonly = "" Then
'		Log($"VLazy.ReadOnly - the readonly for ${mName} has not been set!"$)
'		Return Me
'	End If
...
'disabled
'Sub SetDisabledOnOff(b As Boolean) As VLazy
'	If sDisabled = "" Then
'		Log($"VLazy.Disabled - the disabled for ${mName} has not been set!"$)
'		Return Me
'	End If
