...
Version=8.3
@EndOfDesignText@
'Custom BANano View class: VHover
#IgnoreWarnings:12
...
Private mTagName As String = "v-hover" 
...
Public Sub Initialize (CallBack As Object, Name As String, EventName As String) As VHover 
mName = Name 
...
'set close-delay
Sub SetCloseDelay(varCloseDelay As String) As VHover
...
'set disabled
Sub SetDisabled(varDisabled As Boolean) As VHover
...
'set key
Sub SetKey(varKey As String) As VHover
sKey = varKey
SetAttr("key", sKey)
Return Me
End Sub
...
'set open-delay
Sub SetOpenDelay(varOpenDelay As String) As VHover
...
'set parent-id
Sub SetParentId(varParentId As String) As VHover
...
'set readonly
Sub SetReadonly(varReadonly As String) As VHover
...
'set ref
Sub SetRef(varRef As String) As VHover
sRef = varRef
SetAttr("ref", sRef)
Return Me
End Sub
...
'set required
Sub SetRequired(varRequired As String) As VHover
...
'set v-bind:class
Sub SetVBindClass(varVBindClass As String) As VHover
...
'set v-bind:style
Sub SetVBindStyle(varVBindStyle As String) As VHover
...
'set v-cloak
Sub SetVCloak(varVCloak As Boolean) As VHover
bVCloak = varVCloak
...
'set v-else
Sub SetVElse(varVElse As String) As VHover
sVElse = varVElse
...
'set v-else-if
Sub SetVElseIf(varVElseIf As String) As VHover
sVElseIf = varVElseIf
...
'set v-for
Sub SetVFor(varVFor As String) As VHover
sVFor = varVFor
SetAttr("v-for", sVFor)
Return Me
End Sub
...
'set v-html
Sub SetVHtml(varVHtml As String) As VHover
sVHtml = varVHtml
...
'set v-if
Sub SetVIf(varVIf As String) As VHover
sVIf = varVIf
SetAttr("v-if", sVIf)
Return Me
End Sub
...
'set v-model
Sub SetVModel(varVModel As String) As VHover
sVModel = varVModel
...
'set v-once
Sub SetVOnce(varVOnce As Boolean) As VHover
bVOnce = varVOnce
...
'set v-pre
Sub SetVPre(varVPre As Boolean) As VHover
bVPre = varVPre
SetAttr("v-pre", bVPre)
Return Me
End Sub
...
'set v-show
Sub SetVShow(varVShow As String) As VHover
sVShow = varVShow
...
'set v-text
Sub SetVText(varVText As String) As VHover
sVText = varVText
...
'set value
Sub SetValue(varValue As Boolean) As VHover
bValue = varValue
...
'set border-color
Sub SetBorderColor(varBorderColor As String) As VHover
...
'set border-style
Sub SetBorderStyle(varBorderStyle As String) As VHover
...
'set border-width
Sub SetBorderWidth(varBorderWidth As String) As VHover
...
'set border-radius
Sub SetBorderRadius(varBorderRadius As String) As VHover
...
'set margin-top
Sub SetMarginTop(varMarginTop As String) As VHover
...
'set margin-right
Sub SetMarginRight(varMarginRight As String) As VHover
...
'set margin-bottom
Sub SetMarginBottom(varMarginBottom As String) As VHover
...
'set margin-left
Sub SetMarginLeft(varMarginLeft As String) As VHover
...
'set padding-top
Sub SetPaddingTop(varPaddingTop As String) As VHover
...
'set padding-right
Sub SetPaddingRight(varPaddingRight As String) As VHover
...
'set padding-bottom
Sub SetPaddingBottom(varPaddingBottom As String) As VHover
...
'set padding-left
Sub SetPaddingLeft(varPaddingLeft As String) As VHover
...
Sub AddComponent(child As String) As VHover
...
Sub SetID(varText As String) As VHover
	mname = varText
	Return Me
End Sub
...
public Sub AddToParent(targetID As String) As VHover
...
Sub AddToApp(vap As VueApp) As VHover
	appLink = vap
	data = vap.data	
...
'update the state
Sub SetData(prop as string, value as object) As VHover
	data.put(prop, value)
	Return Me
End Sub
...
'set an event
Sub SetVOn(event As String) As VHover
...
Sub SetCaption(varText As String) As VHover
	If mElement <> Null Then
...
private Sub AddAttr(varName As String, actProp As String) As VHover
...
public Sub SetClasses(Classes As String) As VHover
	If mElement <> Null Then
...
public Sub SetStyle(Style As String) As VHover
	If mElement <> Null Then
...
Sub AddClass(classNames As List) As VHover
...
Sub SetStyles(m As Map) As VHover
...
Sub SetAttrs(props As Map) As VHover
...
'set an attribute
Sub SetAttr(prop As String, value As String) As VHover
...
'bind an attribute
Sub SetVBind(prop As String, value As String) As VHover
...
'set a single style
Sub SetStyleSingle(prop As String, value As String) As VHover
	If BANano.IsUndefined(prop) or BANano.IsNull(prop) Then prop = ""
	If BANano.IsUndefined(value) or BANano.IsNull(value) Then value = ""
	if prop = "" then return me
...
Sub Build(props As Map, styleProps As Map, classNames As List, loose As List) As VHover
	If loose <> Null Then
...
'bind classes
Sub SetVClass(classObj as string) As VHover
	SetVBind("class", classObj)
	Return Me
End Sub

'bind styles
Sub SetVStyle(styleObj as string) As VHover
	SetVBind("style", styleObj)
	Return Me
End Sub

'set color
Sub SetColor1(varColor As String) As VHover
...
'set color intensity
Sub SetColorIntensity(varColor As String, varIntensity As String) As VHover
...
'set text color
Sub SetTextColor1(varColor As String) As VHover
...
Sub SetTextColorIntensity(varColor As String, varIntensity As String) As VHover
	Dim sColor As String = $"${varColor}--text"$
...
'set direct method
Sub SetMethod(Module As Object,methodName As String) As VHover
	methodName = methodName.ToLowerCase
...
'toggle
Sub Toggle As VHover
	If sVShow = "" Then
		Log($"VHover.Toggle - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
Sub Hide As VHover
	If sVShow = "" Then
		Log($"VHover.Hide - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
Sub Show As VHover
	If sVShow = "" Then
		Log($"VHover.Show - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
Sub SetClassOnOff(clsName as string, clsValue As Boolean) As VHover
	if svBindClass = "" then
		Log($"VHover.VBindClass - the v-bind:class for ${mName} has not been set!"$)
		Return Me
	end if
...
'set style 
Sub SetStyleOnOff(styleName as string, styleValue As Boolean) As VHover
	if svBindStyle = "" then
		Log($"VHover.VBindCStyle - the v-bind:style for ${mName} has not been set!"$)
		Return Me
	end if
...
'required
'Sub SetRequiredOnOff(b As Boolean) As VHover
'	If sRequired = "" Then
'		Log($"VHover.Required - the required for ${mName} has not been set!"$)
'		Return Me
'	End If
...
'read only
'Sub SetReadOnlyOnOff(b As Boolean) As VHover
'	If sReadonly = "" Then
'		Log($"VHover.ReadOnly - the readonly for ${mName} has not been set!"$)
'		Return Me
'	End If
...
'disabled
'Sub SetDisabledOnOff(b As Boolean) As VHover
'	If sDisabled = "" Then
'		Log($"VHover.Disabled - the disabled for ${mName} has not been set!"$)
'		Return Me
'	End If
