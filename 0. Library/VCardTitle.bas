...
Version=8.3
@EndOfDesignText@
'Custom BANano View class: VCardTitle
#IgnoreWarnings:12


#DesignerProperty: Key: Caption, DisplayName: Caption, Description: , FieldType: String, DefaultValue: Card Title
...
Private mTagName As String = "v-card-title" 
...
Public Sub Initialize (CallBack As Object, Name As String, EventName As String) As VCardTitle 
mName = Name 
...
'set disabled
Sub SetDisabled(varDisabled As String) As VCardTitle
...
'set key
Sub SetKey(varKey As String) As VCardTitle
sKey = varKey
SetAttr("key", sKey)
Return Me
End Sub
...
'set parent-id
Sub SetParentId(varParentId As String) As VCardTitle
...
'set readonly
Sub SetReadonly(varReadonly As String) As VCardTitle
...
'set ref
Sub SetRef(varRef As String) As VCardTitle
sRef = varRef
SetAttr("ref", sRef)
Return Me
End Sub
...
'set required
Sub SetRequired(varRequired As String) As VCardTitle
...
'set v-bind:class
Sub SetVBindClass(varVBindClass As String) As VCardTitle
...
'set v-bind:style
Sub SetVBindStyle(varVBindStyle As String) As VCardTitle
...
'set v-cloak
Sub SetVCloak(varVCloak As Boolean) As VCardTitle
bVCloak = varVCloak
...
'set v-else
Sub SetVElse(varVElse As String) As VCardTitle
sVElse = varVElse
...
'set v-else-if
Sub SetVElseIf(varVElseIf As String) As VCardTitle
sVElseIf = varVElseIf
...
'set v-for
Sub SetVFor(varVFor As String) As VCardTitle
sVFor = varVFor
SetAttr("v-for", sVFor)
Return Me
End Sub
...
'set v-html
Sub SetVHtml(varVHtml As String) As VCardTitle
sVHtml = varVHtml
...
'set v-if
Sub SetVIf(varVIf As String) As VCardTitle
sVIf = varVIf
SetAttr("v-if", sVIf)
Return Me
End Sub
...
'set v-model
Sub SetVModel(varVModel As String) As VCardTitle
sVModel = varVModel
...
'set v-once
Sub SetVOnce(varVOnce As Boolean) As VCardTitle
bVOnce = varVOnce
...
'set v-pre
Sub SetVPre(varVPre As Boolean) As VCardTitle
bVPre = varVPre
SetAttr("v-pre", bVPre)
Return Me
End Sub
...
'set v-show
Sub SetVShow(varVShow As String) As VCardTitle
sVShow = varVShow
...
'set v-text
Sub SetVText(varVText As String) As VCardTitle
sVText = varVText
...
'set border-color
Sub SetBorderColor(varBorderColor As String) As VCardTitle
...
'set border-style
Sub SetBorderStyle(varBorderStyle As String) As VCardTitle
...
'set border-width
Sub SetBorderWidth(varBorderWidth As String) As VCardTitle
...
'set border-radius
Sub SetBorderRadius(varBorderRadius As String) As VCardTitle
...
'set margin-top
Sub SetMarginTop(varMarginTop As String) As VCardTitle
...
'set margin-right
Sub SetMarginRight(varMarginRight As String) As VCardTitle
...
'set margin-bottom
Sub SetMarginBottom(varMarginBottom As String) As VCardTitle
...
'set margin-left
Sub SetMarginLeft(varMarginLeft As String) As VCardTitle
...
'set padding-top
Sub SetPaddingTop(varPaddingTop As String) As VCardTitle
...
'set padding-right
Sub SetPaddingRight(varPaddingRight As String) As VCardTitle
...
'set padding-bottom
Sub SetPaddingBottom(varPaddingBottom As String) As VCardTitle
...
'set padding-left
Sub SetPaddingLeft(varPaddingLeft As String) As VCardTitle
...
Sub AddComponent(child As String) As VCardTitle
...
Sub SetID(varText As String) As VCardTitle
	mname = varText
	Return Me
End Sub
...
public Sub AddToParent(targetID As String) As VCardTitle
...
Sub AddToApp(vap As VueApp) As VCardTitle
	appLink = vap
	data = vap.data	
...
'update the state
Sub SetData(prop as string, value as object) As VCardTitle
	data.put(prop, value)
	Return Me
End Sub
...
'set an event
Sub SetVOn(event As String) As VCardTitle
...
Sub SetCaption(varText As String) As VCardTitle
	If mElement <> Null Then
...
private Sub AddAttr(varName As String, actProp As String) As VCardTitle
...
public Sub SetClasses(Classes As String) As VCardTitle
	If mElement <> Null Then
...
public Sub SetStyle(Style As String) As VCardTitle
	If mElement <> Null Then
...
Sub AddClass(classNames As List) As VCardTitle
...
Sub SetStyles(m As Map) As VCardTitle
...
Sub SetAttrs(props As Map) As VCardTitle
...
'set an attribute
Sub SetAttr(prop As String, value As String) As VCardTitle
...
'bind an attribute
Sub SetVBind(prop As String, value As String) As VCardTitle
...
'set a single style
Sub SetStyleSingle(prop As String, value As String) As VCardTitle
	If BANano.IsUndefined(prop) or BANano.IsNull(prop) Then prop = ""
	If BANano.IsUndefined(value) or BANano.IsNull(value) Then value = ""
	if prop = "" then return me
...
Sub Build(props As Map, styleProps As Map, classNames As List, loose As List) As VCardTitle
	If loose <> Null Then
...
'bind classes
Sub SetVClass(classObj as string) As VCardTitle
	SetVBind("class", classObj)
	Return Me
End Sub

'bind styles
Sub SetVStyle(styleObj as string) As VCardTitle
	SetVBind("style", styleObj)
	Return Me
End Sub

'set color
Sub SetColor1(varColor As String) As VCardTitle
...
'set color intensity
Sub SetColorIntensity(varColor As String, varIntensity As String) As VCardTitle
...
'set text color
Sub SetTextColor1(varColor As String) As VCardTitle
...
Sub SetTextColorIntensity(varColor As String, varIntensity As String) As VCardTitle
	Dim sColor As String = $"${varColor}--text"$
...
'set direct method
Sub SetMethod(Module As Object,methodName As String) As VCardTitle
	methodName = methodName.ToLowerCase
...
'toggle
Sub Toggle As VCardTitle
	If sVShow = "" Then
		Log($"VCardTitle.Toggle - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
'hide
Sub Hide As VCardTitle
	If sVShow = "" Then
		Log($"VCardTitle.Hide - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
'show
Sub Show As VCardTitle
	If sVShow = "" Then
		Log($"VCardTitle.Show - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
Sub SetClassOnOff(clsName as string, clsValue As Boolean) As VCardTitle
	if svBindClass = "" then
		Log($"VCardTitle.VBindClass - the v-bind:class for ${mName} has not been set!"$)
		Return Me
	end if
...
'set style 
Sub SetStyleOnOff(styleName as string, styleValue As Boolean) As VCardTitle
	if svBindStyle = "" then
		Log($"VCardTitle.VBindCStyle - the v-bind:style for ${mName} has not been set!"$)
		Return Me
	end if
...
'required
'Sub SetRequiredOnOff(b As Boolean) As VCardTitle
'	If sRequired = "" Then
'		Log($"VCardTitle.Required - the required for ${mName} has not been set!"$)
'		Return Me
'	End If
...
'read only
'Sub SetReadOnlyOnOff(b As Boolean) As VCardTitle
'	If sReadonly = "" Then
'		Log($"VCardTitle.ReadOnly - the readonly for ${mName} has not been set!"$)
'		Return Me
'	End If
...
'disabled
'Sub SetDisabledOnOff(b As Boolean) As VCardTitle
'	If sDisabled = "" Then
'		Log($"VCardTitle.Disabled - the disabled for ${mName} has not been set!"$)
'		Return Me
'	End If
