...
Public Sub Initialize (CallBack As Object, Name As String, EventName As String) As VA 
mName = Name 
...
'set disabled
Sub SetDisabled(varDisabled As String) As VA
...
'set download
Sub SetDownload(varDownload As Boolean) As VA
bDownload = varDownload
SetAttr("download", bDownload)
Return Me
End Sub

'set href
Sub SetHref(varHref As String) As VA
sHref = varHref
SetAttr("href", sHref)
Return Me
End Sub
...
'set parent-id
Sub SetParentId(varParentId As String) As VA
...
'set readonly
Sub SetReadonly(varReadonly As String) As VA
...
'set ref
Sub SetRef(varRef As String) As VA
sRef = varRef
SetAttr("ref", sRef)
Return Me
End Sub
...
'set required
Sub SetRequired(varRequired As String) As VA
...
'set target
Sub SetTarget(varTarget As String) As VA
sTarget = varTarget
...
'set title
Sub SetTitle(varTitle As String) As VA
sTitle = varTitle
SetAttr("title", sTitle)
Return Me
End Sub

'set v-bind:class
Sub SetVBindClass(varVBindClass As String) As VA
...
'set v-bind:style
Sub SetVBindStyle(varVBindStyle As String) As VA
...
'set v-else-if
Sub SetVElseIf(varVElseIf As String) As VA
sVElseIf = varVElseIf
...
'set v-for
Sub SetVFor(varVFor As String) As VA
sVFor = varVFor
SetAttr("v-for", sVFor)
Return Me
End Sub
...
'set v-html
Sub SetVHtml(varVHtml As String) As VA
sVHtml = varVHtml
...
'set v-if
Sub SetVIf(varVIf As String) As VA
sVIf = varVIf
SetAttr("v-if", sVIf)
Return Me
End Sub
...
'set v-show
Sub SetVShow(varVShow As String) As VA
sVShow = varVShow
...
'set v-text
Sub SetVText(varVText As String) As VA
sVText = varVText
...
'set border-color
Sub SetBorderColor(varBorderColor As String) As VA
...
'set border-style
Sub SetBorderStyle(varBorderStyle As String) As VA
...
'set border-width
Sub SetBorderWidth(varBorderWidth As String) As VA
...
'set border-radius
Sub SetBorderRadius(varBorderRadius As String) As VA
...
'set margin-top
Sub SetMarginTop(varMarginTop As String) As VA
...
'set margin-right
Sub SetMarginRight(varMarginRight As String) As VA
...
'set margin-bottom
Sub SetMarginBottom(varMarginBottom As String) As VA
...
'set margin-left
Sub SetMarginLeft(varMarginLeft As String) As VA
...
'set padding-top
Sub SetPaddingTop(varPaddingTop As String) As VA
...
'set padding-right
Sub SetPaddingRight(varPaddingRight As String) As VA
...
'set padding-bottom
Sub SetPaddingBottom(varPaddingBottom As String) As VA
...
'set padding-left
Sub SetPaddingLeft(varPaddingLeft As String) As VA
...
Sub AddComponent(child As String) As VA
...
public Sub AddToParent(targetID As String) As VA
...
Sub AddToApp(vap As VueApp) As VA
	appLink = vap
	data = vap.data	
...
'set an event
Sub SetVOn(event As String) As VA
...
Sub SetCaption(varText As String) As VA
	If mElement <> Null Then
...
private Sub AddAttr(varName As String, actProp As String) As VA
...
public Sub SetClasses(Classes As String) As VA
	If mElement <> Null Then
...
public Sub SetStyle(Style As String) As VA
	If mElement <> Null Then
...
Sub AddClass(classNames As List) As VA
...
Sub SetAttrs(props As Map) As VA
...
'set an attribute
Sub SetAttr(prop As String, value As String) As VA
...
'bind an attribute
Sub SetVBind(prop As String, value As String) As VA
...
'set a single style
Sub SetStyleSingle(prop As String, value As String) As VA
	If BANano.IsUndefined(prop) or BANano.IsNull(prop) Then prop = ""
	If BANano.IsUndefined(value) or BANano.IsNull(value) Then value = ""
	if prop = "" then return me
...
Sub Build(props As Map, styleProps As Map, classNames As List, loose As List) As VA
	If loose <> Null Then
...
'set color
Sub SetColor1(varColor As String) As VA
...
'set color intensity
Sub SetColorIntensity(varColor As String, varIntensity As String) As VA
...
Sub SetTextColorIntensity(varColor As String, varIntensity As String) As VA
	Dim sColor As String = $"${varColor}--text"$
...
'set direct method
Sub SetMethod(Module As Object,methodName As String) As VA
	methodName = methodName.ToLowerCase
...
Sub Toggle As VA
	If sVShow = "" Then
		Log($"VA.Toggle - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
Sub Hide As VA
	If sVShow = "" Then
		Log($"VA.Hide - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
Sub Show As VA
	If sVShow = "" Then
		Log($"VA.Show - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
Sub SetClassOnOff(clsName as string, clsValue As Boolean) As VA
	if svBindClass = "" then
		Log($"VA.VBindClass - the v-bind:class for ${mName} has not been set!"$)
		Return Me
	end if
...
'set style 
Sub SetStyleOnOff(styleName as string, styleValue As Boolean) As VA
	if svBindStyle = "" then
		Log($"VA.VBindCStyle - the v-bind:style for ${mName} has not been set!"$)
		Return Me
	end if
...
'required
'Sub SetRequiredOnOff(b As Boolean) As VA
'	If sRequired = "" Then
'		Log($"VA.Required - the required for ${mName} has not been set!"$)
'		Return Me
'	End If
...
'read only
'Sub SetReadOnlyOnOff(b As Boolean) As VA
'	If sReadonly = "" Then
'		Log($"VA.ReadOnly - the readonly for ${mName} has not been set!"$)
'		Return Me
'	End If
...
'disabled
'Sub SetDisabledOnOff(b As Boolean) As VA
'	If sDisabled = "" Then
'		Log($"VA.Disabled - the disabled for ${mName} has not been set!"$)
'		Return Me
'	End If
