...
Version=8.3
@EndOfDesignText@
'Custom BANano View class: VCardText
#IgnoreWarnings:12


#DesignerProperty: Key: Caption, DisplayName: Caption, Description: , FieldType: String, DefaultValue: Card Text
...
Private mTagName As String = "v-card-text" 
...
Public Sub Initialize (CallBack As Object, Name As String, EventName As String) As VCardText 
mName = Name 
...
'set disabled
Sub SetDisabled(varDisabled As String) As VCardText
...
'set key
Sub SetKey(varKey As String) As VCardText
sKey = varKey
SetAttr("key", sKey)
Return Me
End Sub
...
'set parent-id
Sub SetParentId(varParentId As String) As VCardText
...
'set readonly
Sub SetReadonly(varReadonly As String) As VCardText
...
'set ref
Sub SetRef(varRef As String) As VCardText
sRef = varRef
SetAttr("ref", sRef)
Return Me
End Sub
...
'set required
Sub SetRequired(varRequired As String) As VCardText
...
'set v-bind:class
Sub SetVBindClass(varVBindClass As String) As VCardText
...
'set v-bind:style
Sub SetVBindStyle(varVBindStyle As String) As VCardText
...
'set v-cloak
Sub SetVCloak(varVCloak As Boolean) As VCardText
bVCloak = varVCloak
...
'set v-else
Sub SetVElse(varVElse As String) As VCardText
sVElse = varVElse
...
'set v-else-if
Sub SetVElseIf(varVElseIf As String) As VCardText
sVElseIf = varVElseIf
...
'set v-for
Sub SetVFor(varVFor As String) As VCardText
sVFor = varVFor
SetAttr("v-for", sVFor)
Return Me
End Sub
...
'set v-html
Sub SetVHtml(varVHtml As String) As VCardText
sVHtml = varVHtml
...
'set v-if
Sub SetVIf(varVIf As String) As VCardText
sVIf = varVIf
SetAttr("v-if", sVIf)
Return Me
End Sub
...
'set v-model
Sub SetVModel(varVModel As String) As VCardText
sVModel = varVModel
...
'set v-once
Sub SetVOnce(varVOnce As Boolean) As VCardText
bVOnce = varVOnce
...
'set v-pre
Sub SetVPre(varVPre As Boolean) As VCardText
bVPre = varVPre
SetAttr("v-pre", bVPre)
Return Me
End Sub
...
'set v-show
Sub SetVShow(varVShow As String) As VCardText
sVShow = varVShow
...
'set v-text
Sub SetVText(varVText As String) As VCardText
sVText = varVText
...
'set border-color
Sub SetBorderColor(varBorderColor As String) As VCardText
...
'set border-style
Sub SetBorderStyle(varBorderStyle As String) As VCardText
...
'set border-width
Sub SetBorderWidth(varBorderWidth As String) As VCardText
...
'set border-radius
Sub SetBorderRadius(varBorderRadius As String) As VCardText
...
'set margin-top
Sub SetMarginTop(varMarginTop As String) As VCardText
...
'set margin-right
Sub SetMarginRight(varMarginRight As String) As VCardText
...
'set margin-bottom
Sub SetMarginBottom(varMarginBottom As String) As VCardText
...
'set margin-left
Sub SetMarginLeft(varMarginLeft As String) As VCardText
...
'set padding-top
Sub SetPaddingTop(varPaddingTop As String) As VCardText
...
'set padding-right
Sub SetPaddingRight(varPaddingRight As String) As VCardText
...
'set padding-bottom
Sub SetPaddingBottom(varPaddingBottom As String) As VCardText
...
'set padding-left
Sub SetPaddingLeft(varPaddingLeft As String) As VCardText
...
Sub AddComponent(child As String) As VCardText
...
Sub SetID(varText As String) As VCardText
	mname = varText
	Return Me
End Sub
...
public Sub AddToParent(targetID As String) As VCardText
...
Sub AddToApp(vap As VueApp) As VCardText
	appLink = vap
	data = vap.data	
...
'update the state
Sub SetData(prop as string, value as object) As VCardText
	data.put(prop, value)
	Return Me
End Sub
...
'set an event
Sub SetVOn(event As String) As VCardText
...
Sub SetCaption(varText As String) As VCardText
	If mElement <> Null Then
...
private Sub AddAttr(varName As String, actProp As String) As VCardText
...
public Sub SetClasses(Classes As String) As VCardText
	If mElement <> Null Then
...
public Sub SetStyle(Style As String) As VCardText
	If mElement <> Null Then
...
Sub AddClass(classNames As List) As VCardText
...
Sub SetStyles(m As Map) As VCardText
...
Sub SetAttrs(props As Map) As VCardText
...
'set an attribute
Sub SetAttr(prop As String, value As String) As VCardText
...
'bind an attribute
Sub SetVBind(prop As String, value As String) As VCardText
...
'set a single style
Sub SetStyleSingle(prop As String, value As String) As VCardText
	If BANano.IsUndefined(prop) or BANano.IsNull(prop) Then prop = ""
	If BANano.IsUndefined(value) or BANano.IsNull(value) Then value = ""
	if prop = "" then return me
...
Sub Build(props As Map, styleProps As Map, classNames As List, loose As List) As VCardText
	If loose <> Null Then
...
'bind classes
Sub SetVClass(classObj as string) As VCardText
	SetVBind("class", classObj)
	Return Me
End Sub

'bind styles
Sub SetVStyle(styleObj as string) As VCardText
	SetVBind("style", styleObj)
	Return Me
End Sub

'set color
Sub SetColor1(varColor As String) As VCardText
...
'set color intensity
Sub SetColorIntensity(varColor As String, varIntensity As String) As VCardText
...
'set text color
Sub SetTextColor1(varColor As String) As VCardText
...
Sub SetTextColorIntensity(varColor As String, varIntensity As String) As VCardText
	Dim sColor As String = $"${varColor}--text"$
...
'set direct method
Sub SetMethod(Module As Object,methodName As String) As VCardText
	methodName = methodName.ToLowerCase
...
'toggle
Sub Toggle As VCardText
	If sVShow = "" Then
		Log($"VCardText.Toggle - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
'hide
Sub Hide As VCardText
	If sVShow = "" Then
		Log($"VCardText.Hide - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
'show
Sub Show As VCardText
	If sVShow = "" Then
		Log($"VCardText.Show - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
Sub SetClassOnOff(clsName as string, clsValue As Boolean) As VCardText
	if svBindClass = "" then
		Log($"VCardText.VBindClass - the v-bind:class for ${mName} has not been set!"$)
		Return Me
	end if
...
'set style 
Sub SetStyleOnOff(styleName as string, styleValue As Boolean) As VCardText
	if svBindStyle = "" then
		Log($"VCardText.VBindCStyle - the v-bind:style for ${mName} has not been set!"$)
		Return Me
	end if
...
'required
'Sub SetRequiredOnOff(b As Boolean) As VCardText
'	If sRequired = "" Then
'		Log($"VCardText.Required - the required for ${mName} has not been set!"$)
'		Return Me
'	End If
...
'read only
'Sub SetReadOnlyOnOff(b As Boolean) As VCardText
'	If sReadonly = "" Then
'		Log($"VCardText.ReadOnly - the readonly for ${mName} has not been set!"$)
'		Return Me
'	End If
...
'disabled
'Sub SetDisabledOnOff(b As Boolean) As VCardText
'	If sDisabled = "" Then
'		Log($"VCardText.Disabled - the disabled for ${mName} has not been set!"$)
'		Return Me
'	End If
