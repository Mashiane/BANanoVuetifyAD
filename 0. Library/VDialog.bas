...
Sub Class_Globals
	...
	Public sMaxWidth As String = "none"
...
'set transition
Sub SetTransition(varTransition As Object)
	mElement.SetAttr("transition", varTransition)
End Sub
...
'set on clickoutside event
Sub SetOnClickOutside
	Dim mName As String = $"${mEventName}_clickoutside"$
	mName = mName.tolowercase
	If SubExists(mCallBack, mName) = False Then Return
	SetAttr("v-on:click:outside", mName)
