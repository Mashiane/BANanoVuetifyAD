...
Version=8.3
@EndOfDesignText@
'Custom BANano View class: VCardActions
#IgnoreWarnings:12
...
Private mTagName As String = "v-card-actions" 
...
Public Sub Initialize (CallBack As Object, Name As String, EventName As String) As VCardActions 
mName = Name 
...
'set disabled
Sub SetDisabled(varDisabled As String) As VCardActions
...
'set key
Sub SetKey(varKey As String) As VCardActions
sKey = varKey
SetAttr("key", sKey)
Return Me
End Sub
...
'set parent-id
Sub SetParentId(varParentId As String) As VCardActions
...
'set readonly
Sub SetReadonly(varReadonly As String) As VCardActions
...
'set ref
Sub SetRef(varRef As String) As VCardActions
sRef = varRef
SetAttr("ref", sRef)
Return Me
End Sub
...
'set required
Sub SetRequired(varRequired As String) As VCardActions
...
'set v-bind:class
Sub SetVBindClass(varVBindClass As String) As VCardActions
...
'set v-bind:style
Sub SetVBindStyle(varVBindStyle As String) As VCardActions
...
'set v-cloak
Sub SetVCloak(varVCloak As Boolean) As VCardActions
bVCloak = varVCloak
...
'set v-else
Sub SetVElse(varVElse As String) As VCardActions
sVElse = varVElse
...
'set v-else-if
Sub SetVElseIf(varVElseIf As String) As VCardActions
sVElseIf = varVElseIf
...
'set v-for
Sub SetVFor(varVFor As String) As VCardActions
sVFor = varVFor
SetAttr("v-for", sVFor)
Return Me
End Sub
...
'set v-html
Sub SetVHtml(varVHtml As String) As VCardActions
sVHtml = varVHtml
...
'set v-if
Sub SetVIf(varVIf As String) As VCardActions
sVIf = varVIf
SetAttr("v-if", sVIf)
Return Me
End Sub
...
'set v-model
Sub SetVModel(varVModel As String) As VCardActions
sVModel = varVModel
...
'set v-once
Sub SetVOnce(varVOnce As Boolean) As VCardActions
bVOnce = varVOnce
...
'set v-pre
Sub SetVPre(varVPre As Boolean) As VCardActions
bVPre = varVPre
SetAttr("v-pre", bVPre)
Return Me
End Sub
...
'set v-show
Sub SetVShow(varVShow As String) As VCardActions
sVShow = varVShow
...
'set v-text
Sub SetVText(varVText As String) As VCardActions
sVText = varVText
...
'set border-color
Sub SetBorderColor(varBorderColor As String) As VCardActions
...
'set border-style
Sub SetBorderStyle(varBorderStyle As String) As VCardActions
...
'set border-width
Sub SetBorderWidth(varBorderWidth As String) As VCardActions
...
'set border-radius
Sub SetBorderRadius(varBorderRadius As String) As VCardActions
...
'set margin-top
Sub SetMarginTop(varMarginTop As String) As VCardActions
...
'set margin-right
Sub SetMarginRight(varMarginRight As String) As VCardActions
...
'set margin-bottom
Sub SetMarginBottom(varMarginBottom As String) As VCardActions
...
'set margin-left
Sub SetMarginLeft(varMarginLeft As String) As VCardActions
...
'set padding-top
Sub SetPaddingTop(varPaddingTop As String) As VCardActions
...
'set padding-right
Sub SetPaddingRight(varPaddingRight As String) As VCardActions
...
'set padding-bottom
Sub SetPaddingBottom(varPaddingBottom As String) As VCardActions
...
'set padding-left
Sub SetPaddingLeft(varPaddingLeft As String) As VCardActions
...
Sub AddComponent(child As String) As VCardActions
...
Sub SetID(varText As String) As VCardActions
	mname = varText
	Return Me
End Sub
...
public Sub AddToParent(targetID As String) As VCardActions
...
Sub AddToApp(vap As VueApp) As VCardActions
	appLink = vap
	data = vap.data	
...
'update the state
Sub SetData(prop as string, value as object) As VCardActions
	data.put(prop, value)
	Return Me
End Sub
...
'set an event
Sub SetVOn(event As String) As VCardActions
...
Sub SetCaption(varText As String) As VCardActions
	If mElement <> Null Then
...
private Sub AddAttr(varName As String, actProp As String) As VCardActions
...
public Sub SetClasses(Classes As String) As VCardActions
	If mElement <> Null Then
...
public Sub SetStyle(Style As String) As VCardActions
	If mElement <> Null Then
...
Sub AddClass(classNames As List) As VCardActions
...
Sub SetStyles(m As Map) As VCardActions
...
Sub SetAttrs(props As Map) As VCardActions
...
'set an attribute
Sub SetAttr(prop As String, value As String) As VCardActions
...
'bind an attribute
Sub SetVBind(prop As String, value As String) As VCardActions
...
'set a single style
Sub SetStyleSingle(prop As String, value As String) As VCardActions
	If BANano.IsUndefined(prop) or BANano.IsNull(prop) Then prop = ""
	If BANano.IsUndefined(value) or BANano.IsNull(value) Then value = ""
	if prop = "" then return me
...
Sub Build(props As Map, styleProps As Map, classNames As List, loose As List) As VCardActions
	If loose <> Null Then
...
'bind classes
Sub SetVClass(classObj as string) As VCardActions
	SetVBind("class", classObj)
	Return Me
End Sub

'bind styles
Sub SetVStyle(styleObj as string) As VCardActions
	SetVBind("style", styleObj)
	Return Me
End Sub

'set color
Sub SetColor1(varColor As String) As VCardActions
...
'set color intensity
Sub SetColorIntensity(varColor As String, varIntensity As String) As VCardActions
...
'set text color
Sub SetTextColor1(varColor As String) As VCardActions
...
Sub SetTextColorIntensity(varColor As String, varIntensity As String) As VCardActions
	Dim sColor As String = $"${varColor}--text"$
...
'set direct method
Sub SetMethod(Module As Object,methodName As String) As VCardActions
	methodName = methodName.ToLowerCase
...
'toggle
Sub Toggle As VCardActions
	If sVShow = "" Then
		Log($"VCardActions.Toggle - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
'hide
Sub Hide As VCardActions
	If sVShow = "" Then
		Log($"VCardActions.Hide - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
'show
Sub Show As VCardActions
	If sVShow = "" Then
		Log($"VCardActions.Show - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
Sub SetClassOnOff(clsName as string, clsValue As Boolean) As VCardActions
	if svBindClass = "" then
		Log($"VCardActions.VBindClass - the v-bind:class for ${mName} has not been set!"$)
		Return Me
	end if
...
'set style 
Sub SetStyleOnOff(styleName as string, styleValue As Boolean) As VCardActions
	if svBindStyle = "" then
		Log($"VCardActions.VBindCStyle - the v-bind:style for ${mName} has not been set!"$)
		Return Me
	end if
...
'required
'Sub SetRequiredOnOff(b As Boolean) As VCardActions
'	If sRequired = "" Then
'		Log($"VCardActions.Required - the required for ${mName} has not been set!"$)
'		Return Me
'	End If
...
'read only
'Sub SetReadOnlyOnOff(b As Boolean) As VCardActions
'	If sReadonly = "" Then
'		Log($"VCardActions.ReadOnly - the readonly for ${mName} has not been set!"$)
'		Return Me
'	End If
...
'disabled
'Sub SetDisabledOnOff(b As Boolean) As VCardActions
'	If sDisabled = "" Then
'		Log($"VCardActions.Disabled - the disabled for ${mName} has not been set!"$)
'		Return Me
'	End If
