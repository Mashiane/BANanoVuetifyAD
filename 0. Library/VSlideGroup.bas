...
#DesignerProperty: Key: Onclicklocation, DisplayName: Onclicklocation, FieldType: String, DefaultValue: , Description: Event arguments to be passed to the attribute.
...
Private eOnclicklocation As String = ""
...
eOnclicklocation = props.Get("Onclicklocation")
...
SetOnChange
'This activates ClickLocation the event exists on the module
SetOnClickLocation
...
'set disabled
Sub SetDisabled(varDisabled As String) As VSlideGroup
...
'set parent-id
Sub SetParentId(varParentId As String) As VSlideGroup
...
'set v-bind:class
Sub SetVBindClass(varVBindClass As String) As VSlideGroup
...
'set v-bind:style
Sub SetVBindStyle(varVBindStyle As String) As VSlideGroup
...
'set border-color
Sub SetBorderColor(varBorderColor As String) As VSlideGroup
...
'set border-style
Sub SetBorderStyle(varBorderStyle As String) As VSlideGroup
...
'set border-width
Sub SetBorderWidth(varBorderWidth As String) As VSlideGroup
...
'set border-radius
Sub SetBorderRadius(varBorderRadius As String) As VSlideGroup
...
'set margin-top
Sub SetMarginTop(varMarginTop As String) As VSlideGroup
...
'set margin-right
Sub SetMarginRight(varMarginRight As String) As VSlideGroup
...
'set margin-bottom
Sub SetMarginBottom(varMarginBottom As String) As VSlideGroup
...
'set margin-left
Sub SetMarginLeft(varMarginLeft As String) As VSlideGroup
...
'set padding-top
Sub SetPaddingTop(varPaddingTop As String) As VSlideGroup
...
'set padding-right
Sub SetPaddingRight(varPaddingRight As String) As VSlideGroup
...
'set padding-bottom
Sub SetPaddingBottom(varPaddingBottom As String) As VSlideGroup
...
'set padding-left
Sub SetPaddingLeft(varPaddingLeft As String) As VSlideGroup
...
Sub SetOnChangeE(sChange As String) As VSlideGroup
eOnchange = sChange
Return Me
End Sub
...
sName = sName.tolowercase
If SubExists(mCallBack, sName) = False Then Return Me
Dim sCode As String = $"${sName}(${eOnclicklocation})"$
SetAttr("v-on:click:location", sCode)
'arguments for the event
Dim argument As Object 'ignore
Dim cb As BANanoObject = BANano.CallBack(mCallBack, sName, Array(argument))
methods.Put(sName, cb)
...
Sub SetOnClickLocationE(sClickLocation As String) As VSlideGroup
eOnclicklocation = sClickLocation
Return Me
End Sub
...
Sub AddToApp(vap As VueApp) As VSlideGroup
	appLink = vap
	data = vap.data	
...
'update the state
Sub SetData(prop as string, value as object) As VSlideGroup
	data.put(prop, value)
	Return Me
End Sub
...
'bind classes
Sub SetVClass(classObj as string) As VSlideGroup
	SetVBind("class", classObj)
	Return Me
End Sub

'bind styles
Sub SetVStyle(styleObj as string) As VSlideGroup
...
'toggle
Sub Toggle As VSlideGroup
	If sVShow = "" Then
		Log($"VSlideGroup.Toggle - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
'hide
Sub Hide As VSlideGroup
	If sVShow = "" Then
		Log($"VSlideGroup.Hide - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
'show
Sub Show As VSlideGroup
	If sVShow = "" Then
		Log($"VSlideGroup.Show - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
Sub SetClassOnOff(clsName as string, clsValue As Boolean) As VSlideGroup
	if svBindClass = "" then
		Log($"VSlideGroup.VBindClass - the v-bind:class for ${mName} has not been set!"$)
		Return Me
	end if
...
'set style 
Sub SetStyleOnOff(styleName as string, styleValue As Boolean) As VSlideGroup
	if svBindStyle = "" then
		Log($"VSlideGroup.VBindCStyle - the v-bind:style for ${mName} has not been set!"$)
		Return Me
	end if
...
'required
'Sub SetRequiredOnOff(b As Boolean) As VSlideGroup
'	If sRequired = "" Then
'		Log($"VSlideGroup.Required - the required for ${mName} has not been set!"$)
'		Return Me
'	End If
...
'read only
'Sub SetReadOnlyOnOff(b As Boolean) As VSlideGroup
'	If sReadonly = "" Then
'		Log($"VSlideGroup.ReadOnly - the readonly for ${mName} has not been set!"$)
'		Return Me
'	End If
...
'disabled
'Sub SetDisabledOnOff(b As Boolean) As VSlideGroup
'	If sDisabled = "" Then
'		Log($"VSlideGroup.Disabled - the disabled for ${mName} has not been set!"$)
'		Return Me
'	End If
