...
Version=8.3
@EndOfDesignText@
'Custom BANano View class: VContainer
#IgnoreWarnings:12
...
#DesignerProperty: Key: Fluid, DisplayName: Fluid, Description: , FieldType: Boolean, DefaultValue: False
...
Private mTagName As String = "v-container" 
...
Public Sub Initialize (CallBack As Object, Name As String, EventName As String) As VContainer 
mName = Name 
...
'set disabled
Sub SetDisabled(varDisabled As String) As VContainer
...
'set fluid
Sub SetFluid(varFluid As Boolean) As VContainer
bFluid = varFluid
SetAttr("fluid", bFluid)
Return Me
End Sub

'set key
Sub SetKey(varKey As String) As VContainer
sKey = varKey
SetAttr("key", sKey)
Return Me
End Sub
...
'set parent-id
Sub SetParentId(varParentId As String) As VContainer
...
'set readonly
Sub SetReadonly(varReadonly As String) As VContainer
...
'set ref
Sub SetRef(varRef As String) As VContainer
sRef = varRef
SetAttr("ref", sRef)
Return Me
End Sub
...
'set required
Sub SetRequired(varRequired As String) As VContainer
...
'set tag
Sub SetTag(varTag As String) As VContainer
sTag = varTag
SetAttr("tag", sTag)
Return Me
End Sub
...
'set v-bind:class
Sub SetVBindClass(varVBindClass As String) As VContainer
...
'set v-bind:style
Sub SetVBindStyle(varVBindStyle As String) As VContainer
...
'set v-cloak
Sub SetVCloak(varVCloak As Boolean) As VContainer
bVCloak = varVCloak
...
'set v-else
Sub SetVElse(varVElse As String) As VContainer
sVElse = varVElse
...
'set v-else-if
Sub SetVElseIf(varVElseIf As String) As VContainer
sVElseIf = varVElseIf
...
'set v-for
Sub SetVFor(varVFor As String) As VContainer
sVFor = varVFor
SetAttr("v-for", sVFor)
Return Me
End Sub
...
'set v-html
Sub SetVHtml(varVHtml As String) As VContainer
sVHtml = varVHtml
...
'set v-if
Sub SetVIf(varVIf As String) As VContainer
sVIf = varVIf
SetAttr("v-if", sVIf)
Return Me
End Sub
...
'set v-model
Sub SetVModel(varVModel As String) As VContainer
sVModel = varVModel
...
'set v-once
Sub SetVOnce(varVOnce As Boolean) As VContainer
bVOnce = varVOnce
...
'set v-pre
Sub SetVPre(varVPre As Boolean) As VContainer
bVPre = varVPre
SetAttr("v-pre", bVPre)
Return Me
End Sub
...
'set v-show
Sub SetVShow(varVShow As String) As VContainer
sVShow = varVShow
...
'set v-text
Sub SetVText(varVText As String) As VContainer
sVText = varVText
...
'set border-color
Sub SetBorderColor(varBorderColor As String) As VContainer
...
'set border-style
Sub SetBorderStyle(varBorderStyle As String) As VContainer
...
'set border-width
Sub SetBorderWidth(varBorderWidth As String) As VContainer
...
'set border-radius
Sub SetBorderRadius(varBorderRadius As String) As VContainer
...
'set margin-top
Sub SetMarginTop(varMarginTop As String) As VContainer
...
'set margin-right
Sub SetMarginRight(varMarginRight As String) As VContainer
...
'set margin-bottom
Sub SetMarginBottom(varMarginBottom As String) As VContainer
...
'set margin-left
Sub SetMarginLeft(varMarginLeft As String) As VContainer
...
'set padding-top
Sub SetPaddingTop(varPaddingTop As String) As VContainer
...
'set padding-right
Sub SetPaddingRight(varPaddingRight As String) As VContainer
...
'set padding-bottom
Sub SetPaddingBottom(varPaddingBottom As String) As VContainer
...
'set padding-left
Sub SetPaddingLeft(varPaddingLeft As String) As VContainer
...
Sub AddComponent(child As String) As VContainer
...
Sub SetID(varText As String) As VContainer
	mname = varText
	Return Me
End Sub
...
public Sub AddToParent(targetID As String) As VContainer
...
Sub AddToApp(vap As VueApp) As VContainer
	appLink = vap
	data = vap.data	
...
'update the state
Sub SetData(prop as string, value as object) As VContainer
	data.put(prop, value)
	Return Me
End Sub
...
'set an event
Sub SetVOn(event As String) As VContainer
...
Sub SetCaption(varText As String) As VContainer
	If mElement <> Null Then
...
private Sub AddAttr(varName As String, actProp As String) As VContainer
...
public Sub SetClasses(Classes As String) As VContainer
	If mElement <> Null Then
...
public Sub SetStyle(Style As String) As VContainer
	If mElement <> Null Then
...
Sub AddClass(classNames As List) As VContainer
...
Sub SetStyles(m As Map) As VContainer
...
Sub SetAttrs(props As Map) As VContainer
...
'set an attribute
Sub SetAttr(prop As String, value As String) As VContainer
...
'bind an attribute
Sub SetVBind(prop As String, value As String) As VContainer
...
'set a single style
Sub SetStyleSingle(prop As String, value As String) As VContainer
	If BANano.IsUndefined(prop) or BANano.IsNull(prop) Then prop = ""
	If BANano.IsUndefined(value) or BANano.IsNull(value) Then value = ""
	if prop = "" then return me
...
Sub Build(props As Map, styleProps As Map, classNames As List, loose As List) As VContainer
	If loose <> Null Then
...
'bind classes
Sub SetVClass(classObj as string) As VContainer
	SetVBind("class", classObj)
	Return Me
End Sub

'bind styles
Sub SetVStyle(styleObj as string) As VContainer
	SetVBind("style", styleObj)
	Return Me
End Sub

'set color
Sub SetColor1(varColor As String) As VContainer
...
'set color intensity
Sub SetColorIntensity(varColor As String, varIntensity As String) As VContainer
...
'set text color
Sub SetTextColor1(varColor As String) As VContainer
...
Sub SetTextColorIntensity(varColor As String, varIntensity As String) As VContainer
	Dim sColor As String = $"${varColor}--text"$
...
'set direct method
Sub SetMethod(Module As Object,methodName As String) As VContainer
	methodName = methodName.ToLowerCase
...
'toggle
Sub Toggle As VContainer
	If sVShow = "" Then
		Log($"VContainer.Toggle - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
'hide
Sub Hide As VContainer
	If sVShow = "" Then
		Log($"VContainer.Hide - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
'show
Sub Show As VContainer
	If sVShow = "" Then
		Log($"VContainer.Show - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
Sub SetClassOnOff(clsName as string, clsValue As Boolean) As VContainer
	if svBindClass = "" then
		Log($"VContainer.VBindClass - the v-bind:class for ${mName} has not been set!"$)
		Return Me
	end if
...
'set style 
Sub SetStyleOnOff(styleName as string, styleValue As Boolean) As VContainer
	if svBindStyle = "" then
		Log($"VContainer.VBindCStyle - the v-bind:style for ${mName} has not been set!"$)
		Return Me
	end if
...
'required
'Sub SetRequiredOnOff(b As Boolean) As VContainer
'	If sRequired = "" Then
'		Log($"VContainer.Required - the required for ${mName} has not been set!"$)
'		Return Me
'	End If
...
'read only
'Sub SetReadOnlyOnOff(b As Boolean) As VContainer
'	If sReadonly = "" Then
'		Log($"VContainer.ReadOnly - the readonly for ${mName} has not been set!"$)
'		Return Me
'	End If
...
'disabled
'Sub SetDisabledOnOff(b As Boolean) As VContainer
'	If sDisabled = "" Then
'		Log($"VContainer.Disabled - the disabled for ${mName} has not been set!"$)
'		Return Me
'	End If
