...
#DesignerProperty: Key: AutoGrow, DisplayName: AutoGrow, FieldType: Boolean, DefaultValue: false , Description: Set auto-grow
...
#DesignerProperty: Key: NoResize, DisplayName: NoResize, FieldType: Boolean, DefaultValue: false , Description: Set no-resize
...
#DesignerProperty: Key: RowHeight, DisplayName: RowHeight, FieldType: String, DefaultValue: , Description: Set row-height
#DesignerProperty: Key: Rows, DisplayName: Rows, FieldType: String, DefaultValue: , Description: Set rows
...
Sub Class_Globals
	...
	Public bAutoGrow As Boolean = false
	...
	Public bNoResize As Boolean = false
	...
	Public sRowHeight As String = "24"
	Public sRows As String = "5"
...
Public Sub DesignerCreateView (Target As BANanoElement, props As Map)
	...
	If props <> Null Then
		...
		AddAttr(Attributes, bAutoGrow, "b", "auto-grow")
		...
		AddAttr(Attributes, bNoResize, "b", "no-resize")
		...
		AddAttr(Attributes, sRowHeight, "s", "row-height")
		AddAttr(Attributes, sRows, "s", "rows")
...
'set auto-grow
Sub SetAutoGrow(varAutoGrow As Boolean)
	bAutoGrow = varAutoGrow
	mElement.SetAttr("auto-grow", bAutoGrow)
End Sub
...
'set no-resize
Sub SetNoResize(varNoResize As Boolean)
	bNoResize = varNoResize
	mElement.SetAttr("no-resize", bNoResize)
End Sub
...
'set row-height
Sub SetRowHeight(varRowHeight As String)
	sRowHeight = varRowHeight
	mElement.SetAttr("row-height", sRowHeight)
End Sub

'set rows
Sub SetRows(varRows As String)
	sRows = varRows
	mElement.SetAttr("rows", sRows)
End Sub
