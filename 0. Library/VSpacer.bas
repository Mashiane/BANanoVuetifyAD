...
Version=8.3
@EndOfDesignText@
'Custom BANano View class: VSpacer
#IgnoreWarnings:12
...
Private mTagName As String = "v-spacer" 
...
Public Sub Initialize (CallBack As Object, Name As String, EventName As String) As VSpacer 
mName = Name 
...
'set disabled
Sub SetDisabled(varDisabled As String) As VSpacer
...
'set key
Sub SetKey(varKey As String) As VSpacer
sKey = varKey
SetAttr("key", sKey)
Return Me
End Sub
...
'set parent-id
Sub SetParentId(varParentId As String) As VSpacer
...
'set readonly
Sub SetReadonly(varReadonly As String) As VSpacer
...
'set ref
Sub SetRef(varRef As String) As VSpacer
sRef = varRef
SetAttr("ref", sRef)
Return Me
End Sub
...
'set required
Sub SetRequired(varRequired As String) As VSpacer
...
'set v-bind:class
Sub SetVBindClass(varVBindClass As String) As VSpacer
...
'set v-bind:style
Sub SetVBindStyle(varVBindStyle As String) As VSpacer
...
'set v-cloak
Sub SetVCloak(varVCloak As Boolean) As VSpacer
bVCloak = varVCloak
...
'set v-else
Sub SetVElse(varVElse As String) As VSpacer
sVElse = varVElse
...
'set v-else-if
Sub SetVElseIf(varVElseIf As String) As VSpacer
sVElseIf = varVElseIf
...
'set v-for
Sub SetVFor(varVFor As String) As VSpacer
sVFor = varVFor
SetAttr("v-for", sVFor)
Return Me
End Sub
...
'set v-html
Sub SetVHtml(varVHtml As String) As VSpacer
sVHtml = varVHtml
...
'set v-if
Sub SetVIf(varVIf As String) As VSpacer
sVIf = varVIf
SetAttr("v-if", sVIf)
Return Me
End Sub
...
'set v-model
Sub SetVModel(varVModel As String) As VSpacer
sVModel = varVModel
...
'set v-once
Sub SetVOnce(varVOnce As Boolean) As VSpacer
bVOnce = varVOnce
...
'set v-pre
Sub SetVPre(varVPre As Boolean) As VSpacer
bVPre = varVPre
SetAttr("v-pre", bVPre)
Return Me
End Sub
...
'set v-show
Sub SetVShow(varVShow As String) As VSpacer
sVShow = varVShow
...
'set v-text
Sub SetVText(varVText As String) As VSpacer
sVText = varVText
...
'set border-color
Sub SetBorderColor(varBorderColor As String) As VSpacer
...
'set border-style
Sub SetBorderStyle(varBorderStyle As String) As VSpacer
...
'set border-width
Sub SetBorderWidth(varBorderWidth As String) As VSpacer
...
'set border-radius
Sub SetBorderRadius(varBorderRadius As String) As VSpacer
...
'set margin-top
Sub SetMarginTop(varMarginTop As String) As VSpacer
...
'set margin-right
Sub SetMarginRight(varMarginRight As String) As VSpacer
...
'set margin-bottom
Sub SetMarginBottom(varMarginBottom As String) As VSpacer
...
'set margin-left
Sub SetMarginLeft(varMarginLeft As String) As VSpacer
...
'set padding-top
Sub SetPaddingTop(varPaddingTop As String) As VSpacer
...
'set padding-right
Sub SetPaddingRight(varPaddingRight As String) As VSpacer
...
'set padding-bottom
Sub SetPaddingBottom(varPaddingBottom As String) As VSpacer
...
'set padding-left
Sub SetPaddingLeft(varPaddingLeft As String) As VSpacer
...
Sub AddComponent(child As String) As VSpacer
...
Sub SetID(varText As String) As VSpacer
	mname = varText
	Return Me
End Sub
...
public Sub AddToParent(targetID As String) As VSpacer
...
Sub AddToApp(vap As VueApp) As VSpacer
	appLink = vap
	data = vap.data	
...
'update the state
Sub SetData(prop as string, value as object) As VSpacer
	data.put(prop, value)
	Return Me
End Sub
...
'set an event
Sub SetVOn(event As String) As VSpacer
...
Sub SetCaption(varText As String) As VSpacer
	If mElement <> Null Then
...
private Sub AddAttr(varName As String, actProp As String) As VSpacer
...
public Sub SetClasses(Classes As String) As VSpacer
	If mElement <> Null Then
...
public Sub SetStyle(Style As String) As VSpacer
	If mElement <> Null Then
...
Sub AddClass(classNames As List) As VSpacer
...
Sub SetStyles(m As Map) As VSpacer
...
Sub SetAttrs(props As Map) As VSpacer
...
'set an attribute
Sub SetAttr(prop As String, value As String) As VSpacer
...
'bind an attribute
Sub SetVBind(prop As String, value As String) As VSpacer
...
'set a single style
Sub SetStyleSingle(prop As String, value As String) As VSpacer
	If BANano.IsUndefined(prop) or BANano.IsNull(prop) Then prop = ""
	If BANano.IsUndefined(value) or BANano.IsNull(value) Then value = ""
	if prop = "" then return me
...
Sub Build(props As Map, styleProps As Map, classNames As List, loose As List) As VSpacer
	If loose <> Null Then
...
'bind classes
Sub SetVClass(classObj as string) As VSpacer
	SetVBind("class", classObj)
	Return Me
End Sub

'bind styles
Sub SetVStyle(styleObj as string) As VSpacer
	SetVBind("style", styleObj)
	Return Me
End Sub

'set color
Sub SetColor1(varColor As String) As VSpacer
...
'set color intensity
Sub SetColorIntensity(varColor As String, varIntensity As String) As VSpacer
...
'set text color
Sub SetTextColor1(varColor As String) As VSpacer
...
Sub SetTextColorIntensity(varColor As String, varIntensity As String) As VSpacer
	Dim sColor As String = $"${varColor}--text"$
...
'set direct method
Sub SetMethod(Module As Object,methodName As String) As VSpacer
	methodName = methodName.ToLowerCase
...
'toggle
Sub Toggle As VSpacer
	If sVShow = "" Then
		Log($"VSpacer.Toggle - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
Sub Hide As VSpacer
	If sVShow = "" Then
		Log($"VSpacer.Hide - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
Sub Show As VSpacer
	If sVShow = "" Then
		Log($"VSpacer.Show - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
Sub SetClassOnOff(clsName as string, clsValue As Boolean) As VSpacer
	if svBindClass = "" then
		Log($"VSpacer.VBindClass - the v-bind:class for ${mName} has not been set!"$)
		Return Me
	end if
...
'set style 
Sub SetStyleOnOff(styleName as string, styleValue As Boolean) As VSpacer
	if svBindStyle = "" then
		Log($"VSpacer.VBindCStyle - the v-bind:style for ${mName} has not been set!"$)
		Return Me
	end if
...
'required
'Sub SetRequiredOnOff(b As Boolean) As VSpacer
'	If sRequired = "" Then
'		Log($"VSpacer.Required - the required for ${mName} has not been set!"$)
'		Return Me
'	End If
...
'read only
'Sub SetReadOnlyOnOff(b As Boolean) As VSpacer
'	If sReadonly = "" Then
'		Log($"VSpacer.ReadOnly - the readonly for ${mName} has not been set!"$)
'		Return Me
'	End If
...
'disabled
'Sub SetDisabledOnOff(b As Boolean) As VSpacer
'	If sDisabled = "" Then
'		Log($"VSpacer.Disabled - the disabled for ${mName} has not been set!"$)
'		Return Me
'	End If
