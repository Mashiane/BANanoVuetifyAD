...
#DesignerProperty: Key: Circle, DisplayName: Circle, FieldType: Boolean, DefaultValue: false , Description: Set circle
...
#DesignerProperty: Key: TotalVisible, DisplayName: TotalVisible, FieldType: String, DefaultValue: , Description: Set total-visible
...
Public Sub DesignerCreateView (Target As BANanoElement, props As Map)
	...
	If props <> Null Then
		...
		AddAttr(Attributes, bCircle, "b", "circle")
		...
		AddAttr(Attributes, sTotalVisible, "s", "total-visible")
...
'set circle
Sub SetCircle(varCircle As Boolean)
	bCircle = varCircle
	mElement.SetAttr("circle", bCircle)
End Sub
...
Sub SetTotalVisible(varTotalVisible As String)
	...
	mElement.SetAttr("total-visible", sTotalVisible)
End Sub
...
'set on next event
Sub SetOnNext
	Dim mName As String = $"${mEventName}_next"$
	mName = mName.tolowercase
	If SubExists(mCallBack, mName) = False Then Return
	SetAttr("v-on:next", mName)
	Dim argument As Object 'ignore
	Dim cb As BANanoObject = BANano.CallBack(mCallBack, mName, Array(argument))
	methods.Put(mName, cb)
End Sub

'set on previous event
Sub SetOnPrevious
	Dim mName As String = $"${mEventName}_previous"$
	mName = mName.tolowercase
	If SubExists(mCallBack, mName) = False Then Return
	SetAttr("v-on:previous", mName)
	Dim argument As Object 'ignore
	Dim cb As BANanoObject = BANano.CallBack(mCallBack, mName, Array(argument))
	methods.Put(mName, cb)
End Sub
