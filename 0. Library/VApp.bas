...
Private mTagName As String = "v-app" 
...
Public Sub Initialize (CallBack As Object, Name As String, EventName As String) As VApp 
mName = Name 
...
'set dark
Sub SetDark(varDark As Boolean) As VApp
bDark = varDark
SetAttr("dark", bDark)
Return Me
End Sub
...
'set disabled
Sub SetDisabled(varDisabled As String) As VApp
...
'set key
Sub SetKey(varKey As String) As VApp
sKey = varKey
SetAttr("key", sKey)
Return Me
End Sub
...
'set light
Sub SetLight(varLight As Boolean) As VApp
bLight = varLight
...
'set parent-id
Sub SetParentId(varParentId As String) As VApp
...
'set readonly
Sub SetReadonly(varReadonly As String) As VApp
...
'set ref
Sub SetRef(varRef As String) As VApp
sRef = varRef
SetAttr("ref", sRef)
Return Me
End Sub
...
'set required
Sub SetRequired(varRequired As String) As VApp
...
'set v-bind:class
Sub SetVBindClass(varVBindClass As String) As VApp
...
'set v-bind:style
Sub SetVBindStyle(varVBindStyle As String) As VApp
...
'set v-cloak
Sub SetVCloak(varVCloak As Boolean) As VApp
bVCloak = varVCloak
...
'set v-else
Sub SetVElse(varVElse As String) As VApp
sVElse = varVElse
...
'set v-else-if
Sub SetVElseIf(varVElseIf As String) As VApp
sVElseIf = varVElseIf
...
'set v-for
Sub SetVFor(varVFor As String) As VApp
sVFor = varVFor
SetAttr("v-for", sVFor)
Return Me
End Sub
...
'set v-html
Sub SetVHtml(varVHtml As String) As VApp
sVHtml = varVHtml
...
'set v-if
Sub SetVIf(varVIf As String) As VApp
sVIf = varVIf
SetAttr("v-if", sVIf)
Return Me
End Sub
...
'set v-model
Sub SetVModel(varVModel As String) As VApp
sVModel = varVModel
...
'set v-once
Sub SetVOnce(varVOnce As Boolean) As VApp
bVOnce = varVOnce
...
'set v-pre
Sub SetVPre(varVPre As Boolean) As VApp
bVPre = varVPre
SetAttr("v-pre", bVPre)
Return Me
End Sub
...
'set v-show
Sub SetVShow(varVShow As String) As VApp
sVShow = varVShow
...
'set v-text
Sub SetVText(varVText As String) As VApp
sVText = varVText
...
'set border-color
Sub SetBorderColor(varBorderColor As String) As VApp
...
'set border-style
Sub SetBorderStyle(varBorderStyle As String) As VApp
...
'set border-width
Sub SetBorderWidth(varBorderWidth As String) As VApp
...
'set border-radius
Sub SetBorderRadius(varBorderRadius As String) As VApp
...
'set margin-top
Sub SetMarginTop(varMarginTop As String) As VApp
...
'set margin-right
Sub SetMarginRight(varMarginRight As String) As VApp
...
'set margin-bottom
Sub SetMarginBottom(varMarginBottom As String) As VApp
...
'set margin-left
Sub SetMarginLeft(varMarginLeft As String) As VApp
...
'set padding-top
Sub SetPaddingTop(varPaddingTop As String) As VApp
...
'set padding-right
Sub SetPaddingRight(varPaddingRight As String) As VApp
...
'set padding-bottom
Sub SetPaddingBottom(varPaddingBottom As String) As VApp
...
'set padding-left
Sub SetPaddingLeft(varPaddingLeft As String) As VApp
...
Sub AddComponent(child As String) As VApp
...
Sub SetID(varText As String) As VApp
	mname = varText
	Return Me
End Sub
...
public Sub AddToParent(targetID As String) As VApp
...
Sub AddToApp(vap As VueApp) As VApp
	appLink = vap
	data = vap.data	
...
'update the state
Sub SetData(prop as string, value as object) As VApp
	data.put(prop, value)
	Return Me
End Sub
...
'set an event
Sub SetVOn(event As String) As VApp
...
Sub SetCaption(varText As String) As VApp
	If mElement <> Null Then
...
private Sub AddAttr(varName As String, actProp As String) As VApp
...
public Sub SetClasses(Classes As String) As VApp
	If mElement <> Null Then
...
public Sub SetStyle(Style As String) As VApp
	If mElement <> Null Then
...
Sub AddClass(classNames As List) As VApp
...
Sub SetStyles(m As Map) As VApp
...
Sub SetAttrs(props As Map) As VApp
...
'set an attribute
Sub SetAttr(prop As String, value As String) As VApp
...
'bind an attribute
Sub SetVBind(prop As String, value As String) As VApp
...
Sub Build(props As Map, styleProps As Map, classNames As List, loose As List) As VApp
	If loose <> Null Then
...
'bind styles
Sub SetVStyle(styleObj as string) As VApp
	SetVBind("style", styleObj)
	Return Me
End Sub

'set color
Sub SetColor1(varColor As String) As VApp
...
'set color intensity
Sub SetColorIntensity(varColor As String, varIntensity As String) As VApp
...
'set text color
Sub SetTextColor1(varColor As String) As VApp
...
Sub SetTextColorIntensity(varColor As String, varIntensity As String) As VApp
	Dim sColor As String = $"${varColor}--text"$
...
'set direct method
Sub SetMethod(Module As Object,methodName As String) As VApp
	methodName = methodName.ToLowerCase
...
Sub Toggle As VApp
	If sVShow = "" Then
		Log($"VApp.Toggle - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
Sub Hide As VApp
	If sVShow = "" Then
		Log($"VApp.Hide - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
Sub Show As VApp
	If sVShow = "" Then
		Log($"VApp.Show - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
Sub SetClassOnOff(clsName as string, clsValue As Boolean) As VApp
	if svBindClass = "" then
		Log($"VApp.VBindClass - the v-bind:class for ${mName} has not been set!"$)
		Return Me
	end if
...
'set style 
Sub SetStyleOnOff(styleName as string, styleValue As Boolean) As VApp
	if svBindStyle = "" then
		Log($"VApp.VBindCStyle - the v-bind:style for ${mName} has not been set!"$)
		Return Me
	end if
...
'required
'Sub SetRequiredOnOff(b As Boolean) As VApp
'	If sRequired = "" Then
'		Log($"VApp.Required - the required for ${mName} has not been set!"$)
'		Return Me
'	End If
...
'read only
'Sub SetReadOnlyOnOff(b As Boolean) As VApp
'	If sReadonly = "" Then
'		Log($"VApp.ReadOnly - the readonly for ${mName} has not been set!"$)
'		Return Me
'	End If
...
'disabled
'Sub SetDisabledOnOff(b As Boolean) As VApp
'	If sDisabled = "" Then
'		Log($"VApp.Disabled - the disabled for ${mName} has not been set!"$)
'		Return Me
'	End If
