...
#DesignerProperty: Key: Inactive, DisplayName: Inactive, Description: , FieldType: Boolean, DefaultValue: False
...
Private bInactive As Boolean = False
...
Public Sub Initialize (CallBack As Object, Name As String, EventName As String) As VListItem 
mName = Name 
...
'set active-class
Sub SetActiveClass(varActiveClass As String) As VListItem
...
'set append
Sub SetAppend(varAppend As Boolean) As VListItem
bAppend = varAppend
...
'set color
Sub SetColor(varColor As String) As VListItem
sColor = varColor
...
'set dark
Sub SetDark(varDark As Boolean) As VListItem
bDark = varDark
SetAttr("dark", bDark)
Return Me
End Sub
...
'set dense
Sub SetDense(varDense As Boolean) As VListItem
bDense = varDense
...
'set disabled
Sub SetDisabled(varDisabled As Boolean) As VListItem
...
'set exact
Sub SetExact(varExact As Boolean) As VListItem
bExact = varExact
...
Sub SetExactActiveClass(varExactActiveClass As String) As VListItem
...
'set href
Sub SetHref(varHref As String) As VListItem
sHref = varHref
SetAttr("href", sHref)
Return Me
End Sub
...
'set inactive
Sub SetInactive(varInactive As Boolean) As VListItem
bInactive = varInactive
SetAttr("inactive", bInactive)
Return Me
End Sub

'set input-value
Sub SetInputValue(varInputValue As String) As VListItem
...
'set key
Sub SetKey(varKey As String) As VListItem
sKey = varKey
SetAttr("key", sKey)
Return Me
End Sub
...
'set light
Sub SetLight(varLight As Boolean) As VListItem
bLight = varLight
...
'set link
Sub SetLink(varLink As Boolean) As VListItem
bLink = varLink
SetAttr("link", bLink)
Return Me
End Sub
...
'set readonly
Sub SetReadonly(varReadonly As String) As VListItem
...
'set ref
Sub SetRef(varRef As String) As VListItem
sRef = varRef
SetAttr("ref", sRef)
Return Me
End Sub
...
'set replace
Sub SetReplace(varReplace As Boolean) As VListItem
bReplace = varReplace
...
'set required
Sub SetRequired(varRequired As String) As VListItem
...
'set ripple
Sub SetRipple(varRipple As String) As VListItem
sRipple = varRipple
...
'set selectable
Sub SetSelectable(varSelectable As Boolean) As VListItem
...
'set tag
Sub SetTag(varTag As String) As VListItem
sTag = varTag
SetAttr("tag", sTag)
Return Me
End Sub
...
'set target
Sub SetTarget(varTarget As String) As VListItem
sTarget = varTarget
...
'set three-line
Sub SetThreeLine(varThreeLine As Boolean) As VListItem
...
'set to
Sub SetTo(varTo As String) As VListItem
sTo = varTo
SetAttr("to", sTo)
Return Me
End Sub
...
'set two-line
Sub SetTwoLine(varTwoLine As Boolean) As VListItem
bTwoLine = varTwoLine
...
'set v-bind:class
Sub SetVBindClass(varVBindClass As String) As VListItem
...
'set v-bind:style
Sub SetVBindStyle(varVBindStyle As String) As VListItem
...
'set v-cloak
Sub SetVCloak(varVCloak As Boolean) As VListItem
bVCloak = varVCloak
...
'set v-else
Sub SetVElse(varVElse As String) As VListItem
sVElse = varVElse
...
'set v-else-if
Sub SetVElseIf(varVElseIf As String) As VListItem
sVElseIf = varVElseIf
...
'set v-for
Sub SetVFor(varVFor As String) As VListItem
sVFor = varVFor
SetAttr("v-for", sVFor)
Return Me
End Sub
...
'set v-html
Sub SetVHtml(varVHtml As String) As VListItem
sVHtml = varVHtml
...
'set v-if
Sub SetVIf(varVIf As String) As VListItem
sVIf = varVIf
SetAttr("v-if", sVIf)
Return Me
End Sub
...
'set v-model
Sub SetVModel(varVModel As String) As VListItem
sVModel = varVModel
...
'set v-once
Sub SetVOnce(varVOnce As Boolean) As VListItem
bVOnce = varVOnce
...
'set v-pre
Sub SetVPre(varVPre As Boolean) As VListItem
bVPre = varVPre
SetAttr("v-pre", bVPre)
Return Me
End Sub
...
'set v-show
Sub SetVShow(varVShow As String) As VListItem
sVShow = varVShow
...
'set v-text
Sub SetVText(varVText As String) As VListItem
sVText = varVText
...
'set value
Sub SetValue(varValue As String) As VListItem
sValue = varValue
...
'set border-color
Sub SetBorderColor(varBorderColor As String) As VListItem
...
'set border-style
Sub SetBorderStyle(varBorderStyle As String) As VListItem
...
'set border-width
Sub SetBorderWidth(varBorderWidth As String) As VListItem
...
'set border-radius
Sub SetBorderRadius(varBorderRadius As String) As VListItem
...
'set margin-top
Sub SetMarginTop(varMarginTop As String) As VListItem
...
'set margin-right
Sub SetMarginRight(varMarginRight As String) As VListItem
...
'set margin-bottom
Sub SetMarginBottom(varMarginBottom As String) As VListItem
...
'set margin-left
Sub SetMarginLeft(varMarginLeft As String) As VListItem
...
'set padding-top
Sub SetPaddingTop(varPaddingTop As String) As VListItem
...
'set padding-right
Sub SetPaddingRight(varPaddingRight As String) As VListItem
...
'set padding-bottom
Sub SetPaddingBottom(varPaddingBottom As String) As VListItem
...
'set padding-left
Sub SetPaddingLeft(varPaddingLeft As String) As VListItem
...
public Sub AddToParent(targetID As String) As VListItem
...
Sub AddToApp(vap As VueApp) As VListItem
	appLink = vap
	data = vap.data	
...
'update the state
Sub SetData(prop as string, value as object) As VListItem
	data.put(prop, value)
	Return Me
End Sub
...
'set an event
Sub SetVOn(event As String) As VListItem
...
Sub SetCaption(varText As String) As VListItem
	If mElement <> Null Then
...
public Sub SetClasses(Classes As String) As VListItem
	If mElement <> Null Then
...
public Sub SetStyle(Style As String) As VListItem
	If mElement <> Null Then
...
Sub AddClass(classNames As List) As VListItem
...
Sub SetStyles(m As Map) As VListItem
...
Sub SetAttrs(props As Map) As VListItem
...
'set an attribute
Sub SetAttr(prop As String, value As String) As VListItem
...
'bind an attribute
Sub SetVBind(prop As String, value As String) As VListItem
...
'set a single style
Sub SetStyleSingle(prop As String, value As String) As VListItem
	If BANano.IsUndefined(prop) or BANano.IsNull(prop) Then prop = ""
	If BANano.IsUndefined(value) or BANano.IsNull(value) Then value = ""
	if prop = "" then return me
...
Sub Build(props As Map, styleProps As Map, classNames As List, loose As List) As VListItem
	If loose <> Null Then
...
'bind classes
Sub SetVClass(classObj as string) As VListItem
	SetVBind("class", classObj)
	Return Me
End Sub

'bind styles
Sub SetVStyle(styleObj as string) As VListItem
	SetVBind("style", styleObj)
	Return Me
End Sub

'set color
Sub SetColor1(varColor As String) As VListItem
...
'set color intensity
Sub SetColorIntensity(varColor As String, varIntensity As String) As VListItem
...
'set text color
Sub SetTextColor1(varColor As String) As VListItem
...
Sub SetTextColorIntensity(varColor As String, varIntensity As String) As VListItem
	Dim sColor As String = $"${varColor}--text"$
...
'set direct method
Sub SetMethod(Module As Object,methodName As String) As VListItem
	methodName = methodName.ToLowerCase
...
'toggle
Sub Toggle As VListItem
	If sVShow = "" Then
		Log($"VListItem.Toggle - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
'hide
Sub Hide As VListItem
	If sVShow = "" Then
		Log($"VListItem.Hide - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
'show
Sub Show As VListItem
	If sVShow = "" Then
		Log($"VListItem.Show - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
Sub SetClassOnOff(clsName as string, clsValue As Boolean) As VListItem
	if svBindClass = "" then
		Log($"VListItem.VBindClass - the v-bind:class for ${mName} has not been set!"$)
		Return Me
	end if
...
'set style 
Sub SetStyleOnOff(styleName as string, styleValue As Boolean) As VListItem
	if svBindStyle = "" then
		Log($"VListItem.VBindCStyle - the v-bind:style for ${mName} has not been set!"$)
		Return Me
	end if
