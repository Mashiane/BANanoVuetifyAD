2020-07-08 02:39:54 B4J=true
Group=Default Group
ModulesStructureVersion=1
Type=Class
Version=8.3
@EndOfDesignText@
'Custom BANano View class: VThemeProvider
#IgnoreWarnings:12
...
#DesignerProperty: Key: Root, DisplayName: Root, Description: , FieldType: Boolean, DefaultValue: False
...
Private mTagName As String = "v-theme-provider" 
...
Private bRoot As Boolean = False
...
Public Sub Initialize (CallBack As Object, Name As String, EventName As String) As VThemeProvider 
mName = Name 
...
'set dark
Sub SetDark(varDark As Boolean) As VThemeProvider
bDark = varDark
SetAttr("dark", bDark)
Return Me
End Sub
...
'set disabled
Sub SetDisabled(varDisabled As String) As VThemeProvider
...
'set key
Sub SetKey(varKey As String) As VThemeProvider
sKey = varKey
SetAttr("key", sKey)
Return Me
End Sub
...
'set light
Sub SetLight(varLight As Boolean) As VThemeProvider
bLight = varLight
...
'set parent-id
Sub SetParentId(varParentId As String) As VThemeProvider
...
'set readonly
Sub SetReadonly(varReadonly As String) As VThemeProvider
...
'set ref
Sub SetRef(varRef As String) As VThemeProvider
sRef = varRef
SetAttr("ref", sRef)
Return Me
End Sub
...
'set required
Sub SetRequired(varRequired As String) As VThemeProvider
...
'set root
Sub SetRoot(varRoot As Boolean) As VThemeProvider
bRoot = varRoot
SetAttr("root", bRoot)
Return Me
End Sub
...
'set v-bind:class
Sub SetVBindClass(varVBindClass As String) As VThemeProvider
...
'set v-bind:style
Sub SetVBindStyle(varVBindStyle As String) As VThemeProvider
...
'set v-cloak
Sub SetVCloak(varVCloak As Boolean) As VThemeProvider
bVCloak = varVCloak
...
'set v-else
Sub SetVElse(varVElse As String) As VThemeProvider
sVElse = varVElse
...
'set v-else-if
Sub SetVElseIf(varVElseIf As String) As VThemeProvider
sVElseIf = varVElseIf
...
'set v-for
Sub SetVFor(varVFor As String) As VThemeProvider
sVFor = varVFor
SetAttr("v-for", sVFor)
Return Me
End Sub
...
'set v-html
Sub SetVHtml(varVHtml As String) As VThemeProvider
sVHtml = varVHtml
...
'set v-if
Sub SetVIf(varVIf As String) As VThemeProvider
sVIf = varVIf
SetAttr("v-if", sVIf)
Return Me
End Sub
...
'set v-model
Sub SetVModel(varVModel As String) As VThemeProvider
sVModel = varVModel
...
'set v-once
Sub SetVOnce(varVOnce As Boolean) As VThemeProvider
bVOnce = varVOnce
...
'set v-pre
Sub SetVPre(varVPre As Boolean) As VThemeProvider
bVPre = varVPre
SetAttr("v-pre", bVPre)
Return Me
End Sub
...
'set v-show
Sub SetVShow(varVShow As String) As VThemeProvider
sVShow = varVShow
...
'set v-text
Sub SetVText(varVText As String) As VThemeProvider
sVText = varVText
...
'set border-color
Sub SetBorderColor(varBorderColor As String) As VThemeProvider
...
'set border-style
Sub SetBorderStyle(varBorderStyle As String) As VThemeProvider
...
'set border-width
Sub SetBorderWidth(varBorderWidth As String) As VThemeProvider
...
'set border-radius
Sub SetBorderRadius(varBorderRadius As String) As VThemeProvider
...
'set margin-top
Sub SetMarginTop(varMarginTop As String) As VThemeProvider
...
'set margin-right
Sub SetMarginRight(varMarginRight As String) As VThemeProvider
...
'set margin-bottom
Sub SetMarginBottom(varMarginBottom As String) As VThemeProvider
...
'set margin-left
Sub SetMarginLeft(varMarginLeft As String) As VThemeProvider
...
'set padding-top
Sub SetPaddingTop(varPaddingTop As String) As VThemeProvider
...
'set padding-right
Sub SetPaddingRight(varPaddingRight As String) As VThemeProvider
...
'set padding-bottom
Sub SetPaddingBottom(varPaddingBottom As String) As VThemeProvider
...
'set padding-left
Sub SetPaddingLeft(varPaddingLeft As String) As VThemeProvider
...
Sub AddComponent(child As String) As VThemeProvider
...
Sub SetID(varText As String) As VThemeProvider
	mname = varText
	Return Me
End Sub
...
public Sub AddToParent(targetID As String) As VThemeProvider
...
Sub AddToApp(vap As VueApp) As VThemeProvider
	appLink = vap
	data = vap.data	
...
'update the state
Sub SetData(prop as string, value as object) As VThemeProvider
	data.put(prop, value)
	Return Me
End Sub
...
'set an event
Sub SetVOn(event As String) As VThemeProvider
...
Sub SetCaption(varText As String) As VThemeProvider
	If mElement <> Null Then
...
private Sub AddAttr(varName As String, actProp As String) As VThemeProvider
...
public Sub SetClasses(Classes As String) As VThemeProvider
	If mElement <> Null Then
...
public Sub SetStyle(Style As String) As VThemeProvider
	If mElement <> Null Then
...
Sub AddClass(classNames As List) As VThemeProvider
...
Sub SetStyles(m As Map) As VThemeProvider
...
Sub SetAttrs(props As Map) As VThemeProvider
...
'set an attribute
Sub SetAttr(prop As String, value As String) As VThemeProvider
...
'bind an attribute
Sub SetVBind(prop As String, value As String) As VThemeProvider
...
'set a single style
Sub SetStyleSingle(prop As String, value As String) As VThemeProvider
	If BANano.IsUndefined(prop) or BANano.IsNull(prop) Then prop = ""
	If BANano.IsUndefined(value) or BANano.IsNull(value) Then value = ""
	if prop = "" then return me
...
Sub Build(props As Map, styleProps As Map, classNames As List, loose As List) As VThemeProvider
	If loose <> Null Then
...
'bind classes
Sub SetVClass(classObj as string) As VThemeProvider
	SetVBind("class", classObj)
	Return Me
End Sub

'bind styles
Sub SetVStyle(styleObj as string) As VThemeProvider
	SetVBind("style", styleObj)
	Return Me
End Sub

'set color
Sub SetColor1(varColor As String) As VThemeProvider
...
'set color intensity
Sub SetColorIntensity(varColor As String, varIntensity As String) As VThemeProvider
...
'set text color
Sub SetTextColor1(varColor As String) As VThemeProvider
...
Sub SetTextColorIntensity(varColor As String, varIntensity As String) As VThemeProvider
	Dim sColor As String = $"${varColor}--text"$
...
'set direct method
Sub SetMethod(Module As Object,methodName As String) As VThemeProvider
	methodName = methodName.ToLowerCase
...
'toggle
Sub Toggle As VThemeProvider
	If sVShow = "" Then
		Log($"VThemeProvider.Toggle - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
'hide
Sub Hide As VThemeProvider
	If sVShow = "" Then
		Log($"VThemeProvider.Hide - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
'show
Sub Show As VThemeProvider
	If sVShow = "" Then
		Log($"VThemeProvider.Show - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
Sub SetClassOnOff(clsName as string, clsValue As Boolean) As VThemeProvider
	if svBindClass = "" then
		Log($"VThemeProvider.VBindClass - the v-bind:class for ${mName} has not been set!"$)
		Return Me
	end if
...
'set style 
Sub SetStyleOnOff(styleName as string, styleValue As Boolean) As VThemeProvider
	if svBindStyle = "" then
		Log($"VThemeProvider.VBindCStyle - the v-bind:style for ${mName} has not been set!"$)
		Return Me
	end if
...
'required
'Sub SetRequiredOnOff(b As Boolean) As VThemeProvider
'	If sRequired = "" Then
'		Log($"VThemeProvider.Required - the required for ${mName} has not been set!"$)
'		Return Me
'	End If
...
'read only
'Sub SetReadOnlyOnOff(b As Boolean) As VThemeProvider
'	If sReadonly = "" Then
'		Log($"VThemeProvider.ReadOnly - the readonly for ${mName} has not been set!"$)
'		Return Me
'	End If
...
'disabled
'Sub SetDisabledOnOff(b As Boolean) As VThemeProvider
'	If sDisabled = "" Then
'		Log($"VThemeProvider.Disabled - the disabled for ${mName} has not been set!"$)
'		Return Me
'	End If
