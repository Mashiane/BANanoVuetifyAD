2020-07-08 02:39:00 B4J=true
Group=Default Group
ModulesStructureVersion=1
Type=Class
Version=8.3
@EndOfDesignText@
'Custom BANano View class: VPicker
#IgnoreWarnings:12
...
Private mTagName As String = "v-picker" 
...
Public Sub Initialize (CallBack As Object, Name As String, EventName As String) As VPicker 
mName = Name 
...
'set color
Sub SetColor(varColor As String) As VPicker
sColor = varColor
...
'set dark
Sub SetDark(varDark As Boolean) As VPicker
bDark = varDark
SetAttr("dark", bDark)
Return Me
End Sub
...
'set disabled
Sub SetDisabled(varDisabled As String) As VPicker
...
'set elevation
Sub SetElevation(varElevation As String) As VPicker
...
'set flat
Sub SetFlat(varFlat As Boolean) As VPicker
bFlat = varFlat
SetAttr("flat", bFlat)
Return Me
End Sub
...
'set full-width
Sub SetFullWidth(varFullWidth As Boolean) As VPicker
...
'set key
Sub SetKey(varKey As String) As VPicker
sKey = varKey
SetAttr("key", sKey)
Return Me
End Sub
...
'set landscape
Sub SetLandscape(varLandscape As Boolean) As VPicker
...
'set light
Sub SetLight(varLight As Boolean) As VPicker
bLight = varLight
...
'set no-title
Sub SetNoTitle(varNoTitle As Boolean) As VPicker
bNoTitle = varNoTitle
...
'set parent-id
Sub SetParentId(varParentId As String) As VPicker
...
'set readonly
Sub SetReadonly(varReadonly As String) As VPicker
...
'set ref
Sub SetRef(varRef As String) As VPicker
sRef = varRef
SetAttr("ref", sRef)
Return Me
End Sub
...
'set required
Sub SetRequired(varRequired As String) As VPicker
...
'set transition
Sub SetTransition(varTransition As String) As VPicker
...
'set v-bind:class
Sub SetVBindClass(varVBindClass As String) As VPicker
...
'set v-bind:style
Sub SetVBindStyle(varVBindStyle As String) As VPicker
...
'set v-cloak
Sub SetVCloak(varVCloak As Boolean) As VPicker
bVCloak = varVCloak
...
'set v-else
Sub SetVElse(varVElse As String) As VPicker
sVElse = varVElse
...
'set v-else-if
Sub SetVElseIf(varVElseIf As String) As VPicker
sVElseIf = varVElseIf
...
'set v-for
Sub SetVFor(varVFor As String) As VPicker
sVFor = varVFor
SetAttr("v-for", sVFor)
Return Me
End Sub
...
'set v-html
Sub SetVHtml(varVHtml As String) As VPicker
sVHtml = varVHtml
...
'set v-if
Sub SetVIf(varVIf As String) As VPicker
sVIf = varVIf
SetAttr("v-if", sVIf)
Return Me
End Sub
...
'set v-model
Sub SetVModel(varVModel As String) As VPicker
sVModel = varVModel
...
'set v-once
Sub SetVOnce(varVOnce As Boolean) As VPicker
bVOnce = varVOnce
...
'set v-pre
Sub SetVPre(varVPre As Boolean) As VPicker
bVPre = varVPre
SetAttr("v-pre", bVPre)
Return Me
End Sub
...
'set v-show
Sub SetVShow(varVShow As String) As VPicker
sVShow = varVShow
...
'set v-text
Sub SetVText(varVText As String) As VPicker
sVText = varVText
...
'set width
Sub SetWidth(varWidth As String) As VPicker
sWidth = varWidth
...
'set border-color
Sub SetBorderColor(varBorderColor As String) As VPicker
...
'set border-style
Sub SetBorderStyle(varBorderStyle As String) As VPicker
...
'set border-width
Sub SetBorderWidth(varBorderWidth As String) As VPicker
...
'set border-radius
Sub SetBorderRadius(varBorderRadius As String) As VPicker
...
'set margin-top
Sub SetMarginTop(varMarginTop As String) As VPicker
...
'set margin-right
Sub SetMarginRight(varMarginRight As String) As VPicker
...
'set margin-bottom
Sub SetMarginBottom(varMarginBottom As String) As VPicker
...
'set margin-left
Sub SetMarginLeft(varMarginLeft As String) As VPicker
...
'set padding-top
Sub SetPaddingTop(varPaddingTop As String) As VPicker
...
'set padding-right
Sub SetPaddingRight(varPaddingRight As String) As VPicker
...
'set padding-bottom
Sub SetPaddingBottom(varPaddingBottom As String) As VPicker
...
'set padding-left
Sub SetPaddingLeft(varPaddingLeft As String) As VPicker
...
Sub AddComponent(child As String) As VPicker
...
Sub SetID(varText As String) As VPicker
	mname = varText
	Return Me
End Sub
...
public Sub AddToParent(targetID As String) As VPicker
...
Sub AddToApp(vap As VueApp) As VPicker
	appLink = vap
	data = vap.data	
...
'update the state
Sub SetData(prop as string, value as object) As VPicker
	data.put(prop, value)
	Return Me
End Sub
...
'set an event
Sub SetVOn(event As String) As VPicker
...
Sub SetCaption(varText As String) As VPicker
	If mElement <> Null Then
...
private Sub AddAttr(varName As String, actProp As String) As VPicker
...
public Sub SetClasses(Classes As String) As VPicker
	If mElement <> Null Then
...
public Sub SetStyle(Style As String) As VPicker
	If mElement <> Null Then
...
Sub AddClass(classNames As List) As VPicker
...
Sub SetStyles(m As Map) As VPicker
...
Sub SetAttrs(props As Map) As VPicker
...
'set an attribute
Sub SetAttr(prop As String, value As String) As VPicker
...
'bind an attribute
Sub SetVBind(prop As String, value As String) As VPicker
...
'set a single style
Sub SetStyleSingle(prop As String, value As String) As VPicker
	If BANano.IsUndefined(prop) or BANano.IsNull(prop) Then prop = ""
	If BANano.IsUndefined(value) or BANano.IsNull(value) Then value = ""
	if prop = "" then return me
...
Sub Build(props As Map, styleProps As Map, classNames As List, loose As List) As VPicker
	If loose <> Null Then
...
'bind classes
Sub SetVClass(classObj as string) As VPicker
	SetVBind("class", classObj)
	Return Me
End Sub

'bind styles
Sub SetVStyle(styleObj as string) As VPicker
	SetVBind("style", styleObj)
	Return Me
End Sub

'set color
Sub SetColor1(varColor As String) As VPicker
...
'set color intensity
Sub SetColorIntensity(varColor As String, varIntensity As String) As VPicker
...
'set text color
Sub SetTextColor1(varColor As String) As VPicker
...
Sub SetTextColorIntensity(varColor As String, varIntensity As String) As VPicker
	Dim sColor As String = $"${varColor}--text"$
...
'set direct method
Sub SetMethod(Module As Object,methodName As String) As VPicker
	methodName = methodName.ToLowerCase
...
'toggle
Sub Toggle As VPicker
	If sVShow = "" Then
		Log($"VPicker.Toggle - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
Sub Hide As VPicker
	If sVShow = "" Then
		Log($"VPicker.Hide - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
Sub Show As VPicker
	If sVShow = "" Then
		Log($"VPicker.Show - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
Sub SetClassOnOff(clsName as string, clsValue As Boolean) As VPicker
	if svBindClass = "" then
		Log($"VPicker.VBindClass - the v-bind:class for ${mName} has not been set!"$)
		Return Me
	end if
...
'set style 
Sub SetStyleOnOff(styleName as string, styleValue As Boolean) As VPicker
	if svBindStyle = "" then
		Log($"VPicker.VBindCStyle - the v-bind:style for ${mName} has not been set!"$)
		Return Me
	end if
...
'required
'Sub SetRequiredOnOff(b As Boolean) As VPicker
'	If sRequired = "" Then
'		Log($"VPicker.Required - the required for ${mName} has not been set!"$)
'		Return Me
'	End If
...
'read only
'Sub SetReadOnlyOnOff(b As Boolean) As VPicker
'	If sReadonly = "" Then
'		Log($"VPicker.ReadOnly - the readonly for ${mName} has not been set!"$)
'		Return Me
'	End If
...
'disabled
'Sub SetDisabledOnOff(b As Boolean) As VPicker
'	If sDisabled = "" Then
'		Log($"VPicker.Disabled - the disabled for ${mName} has not been set!"$)
'		Return Me
'	End If
