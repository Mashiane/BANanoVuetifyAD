2020-07-08 02:37:48 B4J=true
Group=Default Group
ModulesStructureVersion=1
Type=Class
Version=8.3
@EndOfDesignText@
'Custom BANano View class: VCounter
#IgnoreWarnings:12
...
Private mTagName As String = "v-counter" 
...
Public Sub Initialize (CallBack As Object, Name As String, EventName As String) As VCounter 
mName = Name 
...
'set dark
Sub SetDark(varDark As Boolean) As VCounter
bDark = varDark
SetAttr("dark", bDark)
Return Me
End Sub
...
'set disabled
Sub SetDisabled(varDisabled As String) As VCounter
...
'set key
Sub SetKey(varKey As String) As VCounter
sKey = varKey
SetAttr("key", sKey)
Return Me
End Sub
...
'set light
Sub SetLight(varLight As Boolean) As VCounter
bLight = varLight
...
'set max
Sub SetMax(varMax As String) As VCounter
sMax = varMax
SetAttr("max", sMax)
Return Me
End Sub
...
'set parent-id
Sub SetParentId(varParentId As String) As VCounter
...
'set readonly
Sub SetReadonly(varReadonly As String) As VCounter
...
'set ref
Sub SetRef(varRef As String) As VCounter
sRef = varRef
SetAttr("ref", sRef)
Return Me
End Sub
...
'set required
Sub SetRequired(varRequired As String) As VCounter
...
'set v-bind:class
Sub SetVBindClass(varVBindClass As String) As VCounter
...
'set v-bind:style
Sub SetVBindStyle(varVBindStyle As String) As VCounter
...
'set v-cloak
Sub SetVCloak(varVCloak As Boolean) As VCounter
bVCloak = varVCloak
...
'set v-else
Sub SetVElse(varVElse As String) As VCounter
sVElse = varVElse
...
'set v-else-if
Sub SetVElseIf(varVElseIf As String) As VCounter
sVElseIf = varVElseIf
...
'set v-for
Sub SetVFor(varVFor As String) As VCounter
sVFor = varVFor
SetAttr("v-for", sVFor)
Return Me
End Sub
...
'set v-html
Sub SetVHtml(varVHtml As String) As VCounter
sVHtml = varVHtml
...
'set v-if
Sub SetVIf(varVIf As String) As VCounter
sVIf = varVIf
SetAttr("v-if", sVIf)
Return Me
End Sub
...
'set v-model
Sub SetVModel(varVModel As String) As VCounter
sVModel = varVModel
...
'set v-once
Sub SetVOnce(varVOnce As Boolean) As VCounter
bVOnce = varVOnce
...
'set v-pre
Sub SetVPre(varVPre As Boolean) As VCounter
bVPre = varVPre
SetAttr("v-pre", bVPre)
Return Me
End Sub
...
'set v-show
Sub SetVShow(varVShow As String) As VCounter
sVShow = varVShow
...
'set v-text
Sub SetVText(varVText As String) As VCounter
sVText = varVText
...
'set value
Sub SetValue(varValue As String) As VCounter
sValue = varValue
...
'set border-color
Sub SetBorderColor(varBorderColor As String) As VCounter
...
'set border-style
Sub SetBorderStyle(varBorderStyle As String) As VCounter
...
'set border-width
Sub SetBorderWidth(varBorderWidth As String) As VCounter
...
'set border-radius
Sub SetBorderRadius(varBorderRadius As String) As VCounter
...
'set margin-top
Sub SetMarginTop(varMarginTop As String) As VCounter
...
'set margin-right
Sub SetMarginRight(varMarginRight As String) As VCounter
...
'set margin-bottom
Sub SetMarginBottom(varMarginBottom As String) As VCounter
...
'set margin-left
Sub SetMarginLeft(varMarginLeft As String) As VCounter
...
'set padding-top
Sub SetPaddingTop(varPaddingTop As String) As VCounter
...
'set padding-right
Sub SetPaddingRight(varPaddingRight As String) As VCounter
...
'set padding-bottom
Sub SetPaddingBottom(varPaddingBottom As String) As VCounter
...
'set padding-left
Sub SetPaddingLeft(varPaddingLeft As String) As VCounter
...
Sub AddComponent(child As String) As VCounter
...
Sub SetID(varText As String) As VCounter
	mname = varText
	Return Me
End Sub
...
public Sub AddToParent(targetID As String) As VCounter
...
Sub AddToApp(vap As VueApp) As VCounter
	appLink = vap
	data = vap.data	
...
'update the state
Sub SetData(prop as string, value as object) As VCounter
	data.put(prop, value)
	Return Me
End Sub
...
'set an event
Sub SetVOn(event As String) As VCounter
...
Sub SetCaption(varText As String) As VCounter
	If mElement <> Null Then
...
private Sub AddAttr(varName As String, actProp As String) As VCounter
...
public Sub SetClasses(Classes As String) As VCounter
	If mElement <> Null Then
...
public Sub SetStyle(Style As String) As VCounter
	If mElement <> Null Then
...
Sub AddClass(classNames As List) As VCounter
...
Sub SetStyles(m As Map) As VCounter
...
Sub SetAttrs(props As Map) As VCounter
...
'set an attribute
Sub SetAttr(prop As String, value As String) As VCounter
...
'bind an attribute
Sub SetVBind(prop As String, value As String) As VCounter
...
'set a single style
Sub SetStyleSingle(prop As String, value As String) As VCounter
	If BANano.IsUndefined(prop) or BANano.IsNull(prop) Then prop = ""
	If BANano.IsUndefined(value) or BANano.IsNull(value) Then value = ""
	if prop = "" then return me
...
Sub Build(props As Map, styleProps As Map, classNames As List, loose As List) As VCounter
	If loose <> Null Then
...
'bind classes
Sub SetVClass(classObj as string) As VCounter
	SetVBind("class", classObj)
	Return Me
End Sub

'bind styles
Sub SetVStyle(styleObj as string) As VCounter
	SetVBind("style", styleObj)
	Return Me
End Sub

'set color
Sub SetColor1(varColor As String) As VCounter
...
'set color intensity
Sub SetColorIntensity(varColor As String, varIntensity As String) As VCounter
...
'set text color
Sub SetTextColor1(varColor As String) As VCounter
...
Sub SetTextColorIntensity(varColor As String, varIntensity As String) As VCounter
	Dim sColor As String = $"${varColor}--text"$
...
'set direct method
Sub SetMethod(Module As Object,methodName As String) As VCounter
	methodName = methodName.ToLowerCase
...
'toggle
Sub Toggle As VCounter
	If sVShow = "" Then
		Log($"VCounter.Toggle - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
'hide
Sub Hide As VCounter
	If sVShow = "" Then
		Log($"VCounter.Hide - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
'show
Sub Show As VCounter
	If sVShow = "" Then
		Log($"VCounter.Show - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
Sub SetClassOnOff(clsName as string, clsValue As Boolean) As VCounter
	if svBindClass = "" then
		Log($"VCounter.VBindClass - the v-bind:class for ${mName} has not been set!"$)
		Return Me
	end if
...
'set style 
Sub SetStyleOnOff(styleName as string, styleValue As Boolean) As VCounter
	if svBindStyle = "" then
		Log($"VCounter.VBindCStyle - the v-bind:style for ${mName} has not been set!"$)
		Return Me
	end if
...
'required
'Sub SetRequiredOnOff(b As Boolean) As VCounter
'	If sRequired = "" Then
'		Log($"VCounter.Required - the required for ${mName} has not been set!"$)
'		Return Me
'	End If
...
'read only
'Sub SetReadOnlyOnOff(b As Boolean) As VCounter
'	If sReadonly = "" Then
'		Log($"VCounter.ReadOnly - the readonly for ${mName} has not been set!"$)
'		Return Me
'	End If
...
'disabled
'Sub SetDisabledOnOff(b As Boolean) As VCounter
'	If sDisabled = "" Then
'		Log($"VCounter.Disabled - the disabled for ${mName} has not been set!"$)
'		Return Me
'	End If
