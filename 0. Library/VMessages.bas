2020-06-21 22:46:10 B4J=true
Group=Default Group
ModulesStructureVersion=1
Type=Class
Version=8.3
@EndOfDesignText@
'Custom BANano View class: VMessages
#IgnoreWarnings:12
...
Private mTagName As String = "v-messages" 
...
Public Sub Initialize (CallBack As Object, Name As String, EventName As String) As VMessages 
mName = Name 
...
'set color
Sub SetColor(varColor As String) As VMessages
sColor = varColor
...
'set dark
Sub SetDark(varDark As Boolean) As VMessages
bDark = varDark
SetAttr("dark", bDark)
Return Me
End Sub
...
'set disabled
Sub SetDisabled(varDisabled As String) As VMessages
...
'set key
Sub SetKey(varKey As String) As VMessages
sKey = varKey
SetAttr("key", sKey)
Return Me
End Sub
...
'set light
Sub SetLight(varLight As Boolean) As VMessages
bLight = varLight
...
'set readonly
Sub SetReadonly(varReadonly As String) As VMessages
...
'set ref
Sub SetRef(varRef As String) As VMessages
sRef = varRef
SetAttr("ref", sRef)
Return Me
End Sub
...
'set required
Sub SetRequired(varRequired As String) As VMessages
...
'set v-bind:class
Sub SetVBindClass(varVBindClass As String) As VMessages
...
'set v-bind:style
Sub SetVBindStyle(varVBindStyle As String) As VMessages
...
'set v-cloak
Sub SetVCloak(varVCloak As Boolean) As VMessages
bVCloak = varVCloak
...
'set v-else
Sub SetVElse(varVElse As String) As VMessages
sVElse = varVElse
...
'set v-for
Sub SetVFor(varVFor As String) As VMessages
sVFor = varVFor
SetAttr("v-for", sVFor)
Return Me
End Sub
...
'set v-html
Sub SetVHtml(varVHtml As String) As VMessages
sVHtml = varVHtml
...
'set v-if
Sub SetVIf(varVIf As String) As VMessages
sVIf = varVIf
SetAttr("v-if", sVIf)
Return Me
End Sub
...
'set v-model
Sub SetVModel(varVModel As String) As VMessages
sVModel = varVModel
...
'set v-once
Sub SetVOnce(varVOnce As Boolean) As VMessages
bVOnce = varVOnce
...
'set v-pre
Sub SetVPre(varVPre As Boolean) As VMessages
bVPre = varVPre
SetAttr("v-pre", bVPre)
Return Me
End Sub
...
'set v-show
Sub SetVShow(varVShow As String) As VMessages
sVShow = varVShow
...
'set v-text
Sub SetVText(varVText As String) As VMessages
sVText = varVText
...
'set value
Sub SetValue(varValue As Object) As VMessages
SetAttr("value", varValue)
Return Me
End Sub

'set border-color
Sub SetBorderColor(varBorderColor As String) As VMessages
...
'set border-style
Sub SetBorderStyle(varBorderStyle As String) As VMessages
...
'set border-width
Sub SetBorderWidth(varBorderWidth As String) As VMessages
...
'set border-radius
Sub SetBorderRadius(varBorderRadius As String) As VMessages
...
'set margin-top
Sub SetMarginTop(varMarginTop As String) As VMessages
...
'set margin-right
Sub SetMarginRight(varMarginRight As String) As VMessages
...
'set margin-bottom
Sub SetMarginBottom(varMarginBottom As String) As VMessages
...
'set margin-left
Sub SetMarginLeft(varMarginLeft As String) As VMessages
...
'set padding-top
Sub SetPaddingTop(varPaddingTop As String) As VMessages
...
'set padding-right
Sub SetPaddingRight(varPaddingRight As String) As VMessages
...
'set padding-bottom
Sub SetPaddingBottom(varPaddingBottom As String) As VMessages
...
'set padding-left
Sub SetPaddingLeft(varPaddingLeft As String) As VMessages
...
Sub AddComponent(child As String) As VMessages
...
Sub SetID(varText As String) As VMessages
	mname = varText
	Return Me
End Sub
...
public Sub AddToParent(targetID As String) As VMessages
...
Sub AddToApp(vap As VueApp) As VMessages
	appLink = vap
	data = vap.data	
...
'update the state
Sub SetData(prop as string, value as object) As VMessages
	data.put(prop, value)
	Return Me
End Sub
...
'set an event
Sub SetVOn(event As String) As VMessages
...
Sub SetCaption(varText As String) As VMessages
	If mElement <> Null Then
...
private Sub AddAttr(varName As String, actProp As String) As VMessages
...
public Sub SetClasses(Classes As String) As VMessages
	If mElement <> Null Then
...
public Sub SetStyle(Style As String) As VMessages
	If mElement <> Null Then
...
Sub AddClass(classNames As List) As VMessages
...
Sub SetStyles(m As Map) As VMessages
...
Sub SetAttrs(props As Map) As VMessages
...
'set an attribute
Sub SetAttr(prop As String, value As String) As VMessages
...
'bind an attribute
Sub SetVBind(prop As String, value As String) As VMessages
...
'set a single style
Sub SetStyleSingle(prop As String, value As String) As VMessages
	If BANano.IsUndefined(prop) or BANano.IsNull(prop) Then prop = ""
	If BANano.IsUndefined(value) or BANano.IsNull(value) Then value = ""
	if prop = "" then return me
...
Sub Build(props As Map, styleProps As Map, classNames As List, loose As List) As VMessages
	If loose <> Null Then
...
'bind classes
Sub SetVClass(classObj as string) As VMessages
	SetVBind("class", classObj)
	Return Me
End Sub

'bind styles
Sub SetVStyle(styleObj as string) As VMessages
	SetVBind("style", styleObj)
	Return Me
End Sub

'set color
Sub SetColor1(varColor As String) As VMessages
...
'set color intensity
Sub SetColorIntensity(varColor As String, varIntensity As String) As VMessages
...
'set text color
Sub SetTextColor1(varColor As String) As VMessages
...
Sub SetTextColorIntensity(varColor As String, varIntensity As String) As VMessages
	Dim sColor As String = $"${varColor}--text"$
...
'set direct method
Sub SetMethod(Module As Object,methodName As String) As VMessages
	methodName = methodName.ToLowerCase
...
'toggle
Sub Toggle As VMessages
	If sVShow = "" Then
		Log($"VMessages.Toggle - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
'hide
Sub Hide As VMessages
	If sVShow = "" Then
		Log($"VMessages.Hide - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
'show
Sub Show As VMessages
	If sVShow = "" Then
		Log($"VMessages.Show - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
Sub SetClassOnOff(clsName as string, clsValue As Boolean) As VMessages
	if svBindClass = "" then
		Log($"VMessages.VBindClass - the v-bind:class for ${mName} has not been set!"$)
		Return Me
	end if
...
'set style 
Sub SetStyleOnOff(styleName as string, styleValue As Boolean) As VMessages
	if svBindStyle = "" then
		Log($"VMessages.VBindCStyle - the v-bind:style for ${mName} has not been set!"$)
		Return Me
	end if
...
'required
Sub SetRequiredOnOff(b As Boolean) As VMessages
	If sRequired = "" Then
		Log($"VMessages.Required - the required for ${mName} has not been set!"$)
		Return Me
	End If
...
'read only
Sub SetReadOnlyOnOff(b As Boolean) As VMessages
	If sReadonly = "" Then
		Log($"VMessages.ReadOnly - the readonly for ${mName} has not been set!"$)
		Return Me
	End If
...
'disabled
Sub SetDisabledOnOff(b As Boolean) As VMessages
	If sDisabled = "" Then
		Log($"VMessages.Disabled - the disabled for ${mName} has not been set!"$)
		Return Me
	End If
