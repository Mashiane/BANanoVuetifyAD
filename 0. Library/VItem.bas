2020-07-08 02:38:36 B4J=true
Group=Default Group
ModulesStructureVersion=1
Type=Class
Version=8.3
@EndOfDesignText@
'Custom BANano View class: VItem
#IgnoreWarnings:12
...
Private mTagName As String = "v-item" 
...
Public Sub Initialize (CallBack As Object, Name As String, EventName As String) As VItem 
mName = Name 
...
'set active-class
Sub SetActiveClass(varActiveClass As String) As VItem
...
'set disabled
Sub SetDisabled(varDisabled As Boolean) As VItem
...
'set key
Sub SetKey(varKey As String) As VItem
sKey = varKey
SetAttr("key", sKey)
Return Me
End Sub
...
'set parent-id
Sub SetParentId(varParentId As String) As VItem
...
'set readonly
Sub SetReadonly(varReadonly As String) As VItem
...
'set ref
Sub SetRef(varRef As String) As VItem
sRef = varRef
SetAttr("ref", sRef)
Return Me
End Sub
...
'set required
Sub SetRequired(varRequired As String) As VItem
...
'set v-bind:class
Sub SetVBindClass(varVBindClass As String) As VItem
...
'set v-bind:style
Sub SetVBindStyle(varVBindStyle As String) As VItem
...
'set v-cloak
Sub SetVCloak(varVCloak As Boolean) As VItem
bVCloak = varVCloak
...
'set v-else
Sub SetVElse(varVElse As String) As VItem
sVElse = varVElse
...
'set v-else-if
Sub SetVElseIf(varVElseIf As String) As VItem
sVElseIf = varVElseIf
...
'set v-for
Sub SetVFor(varVFor As String) As VItem
sVFor = varVFor
SetAttr("v-for", sVFor)
Return Me
End Sub
...
'set v-html
Sub SetVHtml(varVHtml As String) As VItem
sVHtml = varVHtml
...
'set v-if
Sub SetVIf(varVIf As String) As VItem
sVIf = varVIf
SetAttr("v-if", sVIf)
Return Me
End Sub
...
'set v-model
Sub SetVModel(varVModel As String) As VItem
sVModel = varVModel
...
'set v-once
Sub SetVOnce(varVOnce As Boolean) As VItem
bVOnce = varVOnce
...
'set v-pre
Sub SetVPre(varVPre As Boolean) As VItem
bVPre = varVPre
SetAttr("v-pre", bVPre)
Return Me
End Sub
...
'set v-show
Sub SetVShow(varVShow As String) As VItem
sVShow = varVShow
...
'set v-text
Sub SetVText(varVText As String) As VItem
sVText = varVText
...
'set value
Sub SetValue(varValue As String) As VItem
sValue = varValue
...
'set border-color
Sub SetBorderColor(varBorderColor As String) As VItem
...
'set border-style
Sub SetBorderStyle(varBorderStyle As String) As VItem
...
'set border-width
Sub SetBorderWidth(varBorderWidth As String) As VItem
...
'set border-radius
Sub SetBorderRadius(varBorderRadius As String) As VItem
...
'set margin-top
Sub SetMarginTop(varMarginTop As String) As VItem
...
'set margin-right
Sub SetMarginRight(varMarginRight As String) As VItem
...
'set margin-bottom
Sub SetMarginBottom(varMarginBottom As String) As VItem
...
'set margin-left
Sub SetMarginLeft(varMarginLeft As String) As VItem
...
'set padding-top
Sub SetPaddingTop(varPaddingTop As String) As VItem
...
'set padding-right
Sub SetPaddingRight(varPaddingRight As String) As VItem
...
'set padding-bottom
Sub SetPaddingBottom(varPaddingBottom As String) As VItem
...
'set padding-left
Sub SetPaddingLeft(varPaddingLeft As String) As VItem
...
Sub AddComponent(child As String) As VItem
...
Sub SetID(varText As String) As VItem
	mname = varText
	Return Me
End Sub
...
public Sub AddToParent(targetID As String) As VItem
...
Sub AddToApp(vap As VueApp) As VItem
	appLink = vap
	data = vap.data	
...
'update the state
Sub SetData(prop as string, value as object) As VItem
	data.put(prop, value)
	Return Me
End Sub
...
'set an event
Sub SetVOn(event As String) As VItem
...
Sub SetCaption(varText As String) As VItem
	If mElement <> Null Then
...
private Sub AddAttr(varName As String, actProp As String) As VItem
...
public Sub SetClasses(Classes As String) As VItem
	If mElement <> Null Then
...
public Sub SetStyle(Style As String) As VItem
	If mElement <> Null Then
...
Sub AddClass(classNames As List) As VItem
...
Sub SetStyles(m As Map) As VItem
...
Sub SetAttrs(props As Map) As VItem
...
'set an attribute
Sub SetAttr(prop As String, value As String) As VItem
...
'bind an attribute
Sub SetVBind(prop As String, value As String) As VItem
...
'set a single style
Sub SetStyleSingle(prop As String, value As String) As VItem
	If BANano.IsUndefined(prop) or BANano.IsNull(prop) Then prop = ""
	If BANano.IsUndefined(value) or BANano.IsNull(value) Then value = ""
	if prop = "" then return me
...
Sub Build(props As Map, styleProps As Map, classNames As List, loose As List) As VItem
	If loose <> Null Then
...
'bind classes
Sub SetVClass(classObj as string) As VItem
	SetVBind("class", classObj)
	Return Me
End Sub

'bind styles
Sub SetVStyle(styleObj as string) As VItem
	SetVBind("style", styleObj)
	Return Me
End Sub

'set color
Sub SetColor1(varColor As String) As VItem
...
'set color intensity
Sub SetColorIntensity(varColor As String, varIntensity As String) As VItem
...
'set text color
Sub SetTextColor1(varColor As String) As VItem
...
Sub SetTextColorIntensity(varColor As String, varIntensity As String) As VItem
	Dim sColor As String = $"${varColor}--text"$
...
'set direct method
Sub SetMethod(Module As Object,methodName As String) As VItem
	methodName = methodName.ToLowerCase
...
Sub Toggle As VItem
	If sVShow = "" Then
		Log($"VItem.Toggle - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
Sub Hide As VItem
	If sVShow = "" Then
		Log($"VItem.Hide - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
Sub Show As VItem
	If sVShow = "" Then
		Log($"VItem.Show - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
Sub SetClassOnOff(clsName as string, clsValue As Boolean) As VItem
	if svBindClass = "" then
		Log($"VItem.VBindClass - the v-bind:class for ${mName} has not been set!"$)
		Return Me
	end if
...
'set style 
Sub SetStyleOnOff(styleName as string, styleValue As Boolean) As VItem
	if svBindStyle = "" then
		Log($"VItem.VBindCStyle - the v-bind:style for ${mName} has not been set!"$)
		Return Me
	end if
...
'required
'Sub SetRequiredOnOff(b As Boolean) As VItem
'	If sRequired = "" Then
'		Log($"VItem.Required - the required for ${mName} has not been set!"$)
'		Return Me
'	End If
...
'read only
'Sub SetReadOnlyOnOff(b As Boolean) As VItem
'	If sReadonly = "" Then
'		Log($"VItem.ReadOnly - the readonly for ${mName} has not been set!"$)
'		Return Me
'	End If
...
'disabled
'Sub SetDisabledOnOff(b As Boolean) As VItem
'	If sDisabled = "" Then
'		Log($"VItem.Disabled - the disabled for ${mName} has not been set!"$)
'		Return Me
'	End If
