...
#DesignerProperty: Key: AltLabels, DisplayName: AltLabels, Description: , FieldType: Boolean, DefaultValue: False
...
#DesignerProperty: Key: NonLinear, DisplayName: NonLinear, Description: , FieldType: Boolean, DefaultValue: False
...
'set disabled
Sub SetDisabled(varDisabled As String) As VStepper
...
'set parent-id
Sub SetParentId(varParentId As String) As VStepper
...
'set v-bind:class
Sub SetVBindClass(varVBindClass As String) As VStepper
...
'set v-bind:style
Sub SetVBindStyle(varVBindStyle As String) As VStepper
...
'set v-else-if
Sub SetVElseIf(varVElseIf As String) As VStepper
sVElseIf = varVElseIf
...
'set border-color
Sub SetBorderColor(varBorderColor As String) As VStepper
...
'set border-style
Sub SetBorderStyle(varBorderStyle As String) As VStepper
...
'set border-width
Sub SetBorderWidth(varBorderWidth As String) As VStepper
...
'set border-radius
Sub SetBorderRadius(varBorderRadius As String) As VStepper
...
'set margin-top
Sub SetMarginTop(varMarginTop As String) As VStepper
...
'set margin-right
Sub SetMarginRight(varMarginRight As String) As VStepper
...
'set margin-bottom
Sub SetMarginBottom(varMarginBottom As String) As VStepper
...
'set margin-left
Sub SetMarginLeft(varMarginLeft As String) As VStepper
...
'set padding-top
Sub SetPaddingTop(varPaddingTop As String) As VStepper
...
'set padding-right
Sub SetPaddingRight(varPaddingRight As String) As VStepper
...
'set padding-bottom
Sub SetPaddingBottom(varPaddingBottom As String) As VStepper
...
'set padding-left
Sub SetPaddingLeft(varPaddingLeft As String) As VStepper
...
Sub SetOnChangeE(sChange As String) As VStepper
eOnchange = sChange
Return Me
End Sub
...
Sub AddToApp(vap As VueApp) As VStepper
	appLink = vap
	data = vap.data	
...
'update the state
Sub SetData(prop as string, value as object) As VStepper
	data.put(prop, value)
	Return Me
End Sub
...
'bind classes
Sub SetVClass(classObj as string) As VStepper
	SetVBind("class", classObj)
	Return Me
End Sub

'bind styles
Sub SetVStyle(styleObj as string) As VStepper
...
'toggle
Sub Toggle As VStepper
	If sVShow = "" Then
		Log($"VStepper.Toggle - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
'hide
Sub Hide As VStepper
	If sVShow = "" Then
		Log($"VStepper.Hide - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
'show
Sub Show As VStepper
	If sVShow = "" Then
		Log($"VStepper.Show - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
Sub SetClassOnOff(clsName as string, clsValue As Boolean) As VStepper
	if svBindClass = "" then
		Log($"VStepper.VBindClass - the v-bind:class for ${mName} has not been set!"$)
		Return Me
	end if
...
'set style 
Sub SetStyleOnOff(styleName as string, styleValue As Boolean) As VStepper
	if svBindStyle = "" then
		Log($"VStepper.VBindCStyle - the v-bind:style for ${mName} has not been set!"$)
		Return Me
	end if
...
'required
'Sub SetRequiredOnOff(b As Boolean) As VStepper
'	If sRequired = "" Then
'		Log($"VStepper.Required - the required for ${mName} has not been set!"$)
'		Return Me
'	End If
...
'read only
'Sub SetReadOnlyOnOff(b As Boolean) As VStepper
'	If sReadonly = "" Then
'		Log($"VStepper.ReadOnly - the readonly for ${mName} has not been set!"$)
'		Return Me
'	End If
...
'disabled
'Sub SetDisabledOnOff(b As Boolean) As VStepper
'	If sDisabled = "" Then
'		Log($"VStepper.Disabled - the disabled for ${mName} has not been set!"$)
'		Return Me
'	End If
