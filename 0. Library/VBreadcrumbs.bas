2020-07-08 02:37:05 B4J=true
Group=Default Group
ModulesStructureVersion=1
Type=Class
Version=8.3
@EndOfDesignText@
'Custom BANano View class: VBreadcrumbs
#IgnoreWarnings:12
...
#DesignerProperty: Key: Divider, DisplayName: Divider, Description: , FieldType: String, DefaultValue: 
...
Private mTagName As String = "v-breadcrumbs" 
...
Public Sub Initialize (CallBack As Object, Name As String, EventName As String) As VBreadcrumbs 
mName = Name 
...
'set dark
Sub SetDark(varDark As Boolean) As VBreadcrumbs
bDark = varDark
SetAttr("dark", bDark)
Return Me
End Sub
...
'set disabled
Sub SetDisabled(varDisabled As String) As VBreadcrumbs
...
'set divider
Sub SetDivider(varDivider As String) As VBreadcrumbs
sDivider = varDivider
SetAttr("divider", sDivider)
Return Me
End Sub

'set items
Sub SetItems(varItems As Object) As VBreadcrumbs
SetAttr("items", varItems)
Return Me
End Sub

'set key
Sub SetKey(varKey As String) As VBreadcrumbs
sKey = varKey
SetAttr("key", sKey)
Return Me
End Sub
...
'set large
Sub SetLarge(varLarge As Boolean) As VBreadcrumbs
bLarge = varLarge
...
'set light
Sub SetLight(varLight As Boolean) As VBreadcrumbs
bLight = varLight
...
'set parent-id
Sub SetParentId(varParentId As String) As VBreadcrumbs
...
'set readonly
Sub SetReadonly(varReadonly As String) As VBreadcrumbs
...
'set ref
Sub SetRef(varRef As String) As VBreadcrumbs
sRef = varRef
SetAttr("ref", sRef)
Return Me
End Sub
...
'set required
Sub SetRequired(varRequired As String) As VBreadcrumbs
...
'set v-bind:class
Sub SetVBindClass(varVBindClass As String) As VBreadcrumbs
...
'set v-bind:style
Sub SetVBindStyle(varVBindStyle As String) As VBreadcrumbs
...
'set v-cloak
Sub SetVCloak(varVCloak As Boolean) As VBreadcrumbs
bVCloak = varVCloak
...
'set v-else
Sub SetVElse(varVElse As String) As VBreadcrumbs
sVElse = varVElse
...
'set v-else-if
Sub SetVElseIf(varVElseIf As String) As VBreadcrumbs
sVElseIf = varVElseIf
...
'set v-for
Sub SetVFor(varVFor As String) As VBreadcrumbs
sVFor = varVFor
SetAttr("v-for", sVFor)
Return Me
End Sub
...
'set v-html
Sub SetVHtml(varVHtml As String) As VBreadcrumbs
sVHtml = varVHtml
...
'set v-if
Sub SetVIf(varVIf As String) As VBreadcrumbs
sVIf = varVIf
SetAttr("v-if", sVIf)
Return Me
End Sub
...
'set v-model
Sub SetVModel(varVModel As String) As VBreadcrumbs
sVModel = varVModel
...
'set v-once
Sub SetVOnce(varVOnce As Boolean) As VBreadcrumbs
bVOnce = varVOnce
...
'set v-pre
Sub SetVPre(varVPre As Boolean) As VBreadcrumbs
bVPre = varVPre
SetAttr("v-pre", bVPre)
Return Me
End Sub
...
'set v-show
Sub SetVShow(varVShow As String) As VBreadcrumbs
sVShow = varVShow
...
'set v-text
Sub SetVText(varVText As String) As VBreadcrumbs
sVText = varVText
...
'set border-color
Sub SetBorderColor(varBorderColor As String) As VBreadcrumbs
...
'set border-style
Sub SetBorderStyle(varBorderStyle As String) As VBreadcrumbs
...
'set border-width
Sub SetBorderWidth(varBorderWidth As String) As VBreadcrumbs
...
'set border-radius
Sub SetBorderRadius(varBorderRadius As String) As VBreadcrumbs
...
'set margin-top
Sub SetMarginTop(varMarginTop As String) As VBreadcrumbs
...
'set margin-right
Sub SetMarginRight(varMarginRight As String) As VBreadcrumbs
...
'set margin-bottom
Sub SetMarginBottom(varMarginBottom As String) As VBreadcrumbs
...
'set margin-left
Sub SetMarginLeft(varMarginLeft As String) As VBreadcrumbs
...
'set padding-top
Sub SetPaddingTop(varPaddingTop As String) As VBreadcrumbs
...
'set padding-right
Sub SetPaddingRight(varPaddingRight As String) As VBreadcrumbs
...
'set padding-bottom
Sub SetPaddingBottom(varPaddingBottom As String) As VBreadcrumbs
...
'set padding-left
Sub SetPaddingLeft(varPaddingLeft As String) As VBreadcrumbs
...
Sub AddComponent(child As String) As VBreadcrumbs
...
Sub SetID(varText As String) As VBreadcrumbs
	mname = varText
	Return Me
End Sub
...
public Sub AddToParent(targetID As String) As VBreadcrumbs
...
Sub AddToApp(vap As VueApp) As VBreadcrumbs
	appLink = vap
	data = vap.data	
...
'update the state
Sub SetData(prop as string, value as object) As VBreadcrumbs
	data.put(prop, value)
	Return Me
End Sub
...
'set an event
Sub SetVOn(event As String) As VBreadcrumbs
...
Sub SetCaption(varText As String) As VBreadcrumbs
	If mElement <> Null Then
...
private Sub AddAttr(varName As String, actProp As String) As VBreadcrumbs
...
public Sub SetClasses(Classes As String) As VBreadcrumbs
	If mElement <> Null Then
...
public Sub SetStyle(Style As String) As VBreadcrumbs
	If mElement <> Null Then
...
Sub AddClass(classNames As List) As VBreadcrumbs
...
Sub SetStyles(m As Map) As VBreadcrumbs
...
Sub SetAttrs(props As Map) As VBreadcrumbs
...
'set an attribute
Sub SetAttr(prop As String, value As String) As VBreadcrumbs
...
'bind an attribute
Sub SetVBind(prop As String, value As String) As VBreadcrumbs
...
'set a single style
Sub SetStyleSingle(prop As String, value As String) As VBreadcrumbs
	If BANano.IsUndefined(prop) or BANano.IsNull(prop) Then prop = ""
	If BANano.IsUndefined(value) or BANano.IsNull(value) Then value = ""
	if prop = "" then return me
...
Sub Build(props As Map, styleProps As Map, classNames As List, loose As List) As VBreadcrumbs
	If loose <> Null Then
...
'bind classes
Sub SetVClass(classObj as string) As VBreadcrumbs
	SetVBind("class", classObj)
	Return Me
End Sub

'bind styles
Sub SetVStyle(styleObj as string) As VBreadcrumbs
	SetVBind("style", styleObj)
	Return Me
End Sub

'set color
Sub SetColor1(varColor As String) As VBreadcrumbs
...
'set color intensity
Sub SetColorIntensity(varColor As String, varIntensity As String) As VBreadcrumbs
...
'set text color
Sub SetTextColor1(varColor As String) As VBreadcrumbs
...
Sub SetTextColorIntensity(varColor As String, varIntensity As String) As VBreadcrumbs
	Dim sColor As String = $"${varColor}--text"$
...
'set direct method
Sub SetMethod(Module As Object,methodName As String) As VBreadcrumbs
	methodName = methodName.ToLowerCase
...
'toggle
Sub Toggle As VBreadcrumbs
	If sVShow = "" Then
		Log($"VBreadcrumbs.Toggle - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
'hide
Sub Hide As VBreadcrumbs
	If sVShow = "" Then
		Log($"VBreadcrumbs.Hide - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
'show
Sub Show As VBreadcrumbs
	If sVShow = "" Then
		Log($"VBreadcrumbs.Show - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
Sub SetClassOnOff(clsName as string, clsValue As Boolean) As VBreadcrumbs
	if svBindClass = "" then
		Log($"VBreadcrumbs.VBindClass - the v-bind:class for ${mName} has not been set!"$)
		Return Me
	end if
...
'set style 
Sub SetStyleOnOff(styleName as string, styleValue As Boolean) As VBreadcrumbs
	if svBindStyle = "" then
		Log($"VBreadcrumbs.VBindCStyle - the v-bind:style for ${mName} has not been set!"$)
		Return Me
	end if
...
'required
'Sub SetRequiredOnOff(b As Boolean) As VBreadcrumbs
'	If sRequired = "" Then
'		Log($"VBreadcrumbs.Required - the required for ${mName} has not been set!"$)
'		Return Me
'	End If
...
'read only
'Sub SetReadOnlyOnOff(b As Boolean) As VBreadcrumbs
'	If sReadonly = "" Then
'		Log($"VBreadcrumbs.ReadOnly - the readonly for ${mName} has not been set!"$)
'		Return Me
'	End If
...
'disabled
'Sub SetDisabledOnOff(b As Boolean) As VBreadcrumbs
'	If sDisabled = "" Then
'		Log($"VBreadcrumbs.Disabled - the disabled for ${mName} has not been set!"$)
'		Return Me
'	End If
