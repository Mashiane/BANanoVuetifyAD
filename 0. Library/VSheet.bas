...
Version=8.3
@EndOfDesignText@
'Custom BANano View class: VSheet
#IgnoreWarnings:12
...
Private mTagName As String = "v-sheet" 
...
Public Sub Initialize (CallBack As Object, Name As String, EventName As String) As VSheet 
mName = Name 
...
'set color
Sub SetColor(varColor As String) As VSheet
sColor = varColor
...
'set dark
Sub SetDark(varDark As Boolean) As VSheet
bDark = varDark
SetAttr("dark", bDark)
Return Me
End Sub
...
'set disabled
Sub SetDisabled(varDisabled As String) As VSheet
...
'set elevation
Sub SetElevation(varElevation As String) As VSheet
...
'set height
Sub SetHeight(varHeight As String) As VSheet
sHeight = varHeight
...
'set key
Sub SetKey(varKey As String) As VSheet
sKey = varKey
SetAttr("key", sKey)
Return Me
End Sub
...
'set light
Sub SetLight(varLight As Boolean) As VSheet
bLight = varLight
...
'set max-height
Sub SetMaxHeight(varMaxHeight As String) As VSheet
...
'set max-width
Sub SetMaxWidth(varMaxWidth As String) As VSheet
...
'set min-height
Sub SetMinHeight(varMinHeight As String) As VSheet
...
'set min-width
Sub SetMinWidth(varMinWidth As String) As VSheet
...
'set outlined
Sub SetOutlined(varOutlined As Boolean) As VSheet
...
'set parent-id
Sub SetParentId(varParentId As String) As VSheet
...
'set readonly
Sub SetReadonly(varReadonly As String) As VSheet
...
'set ref
Sub SetRef(varRef As String) As VSheet
sRef = varRef
SetAttr("ref", sRef)
Return Me
End Sub
...
'set required
Sub SetRequired(varRequired As String) As VSheet
...
'set rounded
Sub SetRounded(varRounded As String) As VSheet
sRounded = varRounded
...
'set shaped
Sub SetShaped(varShaped As Boolean) As VSheet
bShaped = varShaped
...
'set tag
Sub SetTag(varTag As String) As VSheet
sTag = varTag
SetAttr("tag", sTag)
Return Me
End Sub
...
'set tile
Sub SetTile(varTile As Boolean) As VSheet
bTile = varTile
SetAttr("tile", bTile)
Return Me
End Sub
...
'set v-bind:class
Sub SetVBindClass(varVBindClass As String) As VSheet
...
'set v-bind:style
Sub SetVBindStyle(varVBindStyle As String) As VSheet
...
'set v-cloak
Sub SetVCloak(varVCloak As Boolean) As VSheet
bVCloak = varVCloak
...
'set v-else
Sub SetVElse(varVElse As String) As VSheet
sVElse = varVElse
...
'set v-else-if
Sub SetVElseIf(varVElseIf As String) As VSheet
sVElseIf = varVElseIf
...
'set v-for
Sub SetVFor(varVFor As String) As VSheet
sVFor = varVFor
SetAttr("v-for", sVFor)
Return Me
End Sub
...
'set v-html
Sub SetVHtml(varVHtml As String) As VSheet
sVHtml = varVHtml
...
'set v-if
Sub SetVIf(varVIf As String) As VSheet
sVIf = varVIf
SetAttr("v-if", sVIf)
Return Me
End Sub
...
'set v-model
Sub SetVModel(varVModel As String) As VSheet
sVModel = varVModel
...
'set v-once
Sub SetVOnce(varVOnce As Boolean) As VSheet
bVOnce = varVOnce
...
'set v-pre
Sub SetVPre(varVPre As Boolean) As VSheet
bVPre = varVPre
SetAttr("v-pre", bVPre)
Return Me
End Sub
...
'set v-show
Sub SetVShow(varVShow As String) As VSheet
sVShow = varVShow
...
'set v-text
Sub SetVText(varVText As String) As VSheet
sVText = varVText
...
'set width
Sub SetWidth(varWidth As String) As VSheet
sWidth = varWidth
...
'set border-color
Sub SetBorderColor(varBorderColor As String) As VSheet
...
'set border-style
Sub SetBorderStyle(varBorderStyle As String) As VSheet
...
'set border-width
Sub SetBorderWidth(varBorderWidth As String) As VSheet
...
'set border-radius
Sub SetBorderRadius(varBorderRadius As String) As VSheet
...
'set margin-top
Sub SetMarginTop(varMarginTop As String) As VSheet
...
'set margin-right
Sub SetMarginRight(varMarginRight As String) As VSheet
...
'set margin-bottom
Sub SetMarginBottom(varMarginBottom As String) As VSheet
...
'set margin-left
Sub SetMarginLeft(varMarginLeft As String) As VSheet
...
'set padding-top
Sub SetPaddingTop(varPaddingTop As String) As VSheet
...
'set padding-right
Sub SetPaddingRight(varPaddingRight As String) As VSheet
...
'set padding-bottom
Sub SetPaddingBottom(varPaddingBottom As String) As VSheet
...
'set padding-left
Sub SetPaddingLeft(varPaddingLeft As String) As VSheet
...
Sub AddComponent(child As String) As VSheet
...
Sub SetID(varText As String) As VSheet
	mname = varText
	Return Me
End Sub
...
public Sub AddToParent(targetID As String) As VSheet
...
Sub AddToApp(vap As VueApp) As VSheet
	appLink = vap
	data = vap.data	
...
'update the state
Sub SetData(prop as string, value as object) As VSheet
	data.put(prop, value)
	Return Me
End Sub
...
'set an event
Sub SetVOn(event As String) As VSheet
...
Sub SetCaption(varText As String) As VSheet
	If mElement <> Null Then
...
private Sub AddAttr(varName As String, actProp As String) As VSheet
...
public Sub SetClasses(Classes As String) As VSheet
	If mElement <> Null Then
...
public Sub SetStyle(Style As String) As VSheet
	If mElement <> Null Then
...
Sub AddClass(classNames As List) As VSheet
...
Sub SetStyles(m As Map) As VSheet
...
Sub SetAttrs(props As Map) As VSheet
...
'set an attribute
Sub SetAttr(prop As String, value As String) As VSheet
...
'bind an attribute
Sub SetVBind(prop As String, value As String) As VSheet
...
'set a single style
Sub SetStyleSingle(prop As String, value As String) As VSheet
	If BANano.IsUndefined(prop) or BANano.IsNull(prop) Then prop = ""
	If BANano.IsUndefined(value) or BANano.IsNull(value) Then value = ""
	if prop = "" then return me
...
Sub Build(props As Map, styleProps As Map, classNames As List, loose As List) As VSheet
	If loose <> Null Then
...
'bind classes
Sub SetVClass(classObj as string) As VSheet
	SetVBind("class", classObj)
	Return Me
End Sub

'bind styles
Sub SetVStyle(styleObj as string) As VSheet
	SetVBind("style", styleObj)
	Return Me
End Sub

'set color
Sub SetColor1(varColor As String) As VSheet
...
'set color intensity
Sub SetColorIntensity(varColor As String, varIntensity As String) As VSheet
...
'set text color
Sub SetTextColor1(varColor As String) As VSheet
...
Sub SetTextColorIntensity(varColor As String, varIntensity As String) As VSheet
	Dim sColor As String = $"${varColor}--text"$
...
'set direct method
Sub SetMethod(Module As Object,methodName As String) As VSheet
	methodName = methodName.ToLowerCase
...
'toggle
Sub Toggle As VSheet
	If sVShow = "" Then
		Log($"VSheet.Toggle - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
Sub Hide As VSheet
	If sVShow = "" Then
		Log($"VSheet.Hide - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
Sub Show As VSheet
	If sVShow = "" Then
		Log($"VSheet.Show - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
Sub SetClassOnOff(clsName as string, clsValue As Boolean) As VSheet
	if svBindClass = "" then
		Log($"VSheet.VBindClass - the v-bind:class for ${mName} has not been set!"$)
		Return Me
	end if
...
'set style 
Sub SetStyleOnOff(styleName as string, styleValue As Boolean) As VSheet
	if svBindStyle = "" then
		Log($"VSheet.VBindCStyle - the v-bind:style for ${mName} has not been set!"$)
		Return Me
	end if
...
'required
'Sub SetRequiredOnOff(b As Boolean) As VSheet
'	If sRequired = "" Then
'		Log($"VSheet.Required - the required for ${mName} has not been set!"$)
'		Return Me
'	End If
...
'read only
'Sub SetReadOnlyOnOff(b As Boolean) As VSheet
'	If sReadonly = "" Then
'		Log($"VSheet.ReadOnly - the readonly for ${mName} has not been set!"$)
'		Return Me
'	End If
...
'disabled
'Sub SetDisabledOnOff(b As Boolean) As VSheet
'	If sDisabled = "" Then
'		Log($"VSheet.Disabled - the disabled for ${mName} has not been set!"$)
'		Return Me
'	End If
