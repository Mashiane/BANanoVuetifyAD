2020-07-08 02:39:47 B4J=true
Group=Default Group
ModulesStructureVersion=1
Type=Class
Version=8.3
@EndOfDesignText@
'Custom BANano View class: VTableOverflow
#IgnoreWarnings:12
...
Private mTagName As String = "v-table-overflow" 
...
Public Sub Initialize (CallBack As Object, Name As String, EventName As String) As VTableOverflow 
mName = Name 
...
'set disabled
Sub SetDisabled(varDisabled As String) As VTableOverflow
...
'set key
Sub SetKey(varKey As String) As VTableOverflow
sKey = varKey
SetAttr("key", sKey)
Return Me
End Sub
...
'set parent-id
Sub SetParentId(varParentId As String) As VTableOverflow
...
'set readonly
Sub SetReadonly(varReadonly As String) As VTableOverflow
...
'set ref
Sub SetRef(varRef As String) As VTableOverflow
sRef = varRef
SetAttr("ref", sRef)
Return Me
End Sub
...
'set required
Sub SetRequired(varRequired As String) As VTableOverflow
...
'set v-bind:class
Sub SetVBindClass(varVBindClass As String) As VTableOverflow
...
'set v-bind:style
Sub SetVBindStyle(varVBindStyle As String) As VTableOverflow
...
'set v-cloak
Sub SetVCloak(varVCloak As Boolean) As VTableOverflow
bVCloak = varVCloak
...
'set v-else
Sub SetVElse(varVElse As String) As VTableOverflow
sVElse = varVElse
...
'set v-else-if
Sub SetVElseIf(varVElseIf As String) As VTableOverflow
sVElseIf = varVElseIf
...
'set v-for
Sub SetVFor(varVFor As String) As VTableOverflow
sVFor = varVFor
SetAttr("v-for", sVFor)
Return Me
End Sub
...
'set v-html
Sub SetVHtml(varVHtml As String) As VTableOverflow
sVHtml = varVHtml
...
'set v-if
Sub SetVIf(varVIf As String) As VTableOverflow
sVIf = varVIf
SetAttr("v-if", sVIf)
Return Me
End Sub
...
'set v-model
Sub SetVModel(varVModel As String) As VTableOverflow
sVModel = varVModel
...
'set v-once
Sub SetVOnce(varVOnce As Boolean) As VTableOverflow
bVOnce = varVOnce
...
'set v-pre
Sub SetVPre(varVPre As Boolean) As VTableOverflow
bVPre = varVPre
SetAttr("v-pre", bVPre)
Return Me
End Sub
...
'set v-show
Sub SetVShow(varVShow As String) As VTableOverflow
sVShow = varVShow
...
'set v-text
Sub SetVText(varVText As String) As VTableOverflow
sVText = varVText
...
'set border-color
Sub SetBorderColor(varBorderColor As String) As VTableOverflow
...
'set border-style
Sub SetBorderStyle(varBorderStyle As String) As VTableOverflow
...
'set border-width
Sub SetBorderWidth(varBorderWidth As String) As VTableOverflow
...
'set border-radius
Sub SetBorderRadius(varBorderRadius As String) As VTableOverflow
...
'set margin-top
Sub SetMarginTop(varMarginTop As String) As VTableOverflow
...
'set margin-right
Sub SetMarginRight(varMarginRight As String) As VTableOverflow
...
'set margin-bottom
Sub SetMarginBottom(varMarginBottom As String) As VTableOverflow
...
'set margin-left
Sub SetMarginLeft(varMarginLeft As String) As VTableOverflow
...
'set padding-top
Sub SetPaddingTop(varPaddingTop As String) As VTableOverflow
...
'set padding-right
Sub SetPaddingRight(varPaddingRight As String) As VTableOverflow
...
'set padding-bottom
Sub SetPaddingBottom(varPaddingBottom As String) As VTableOverflow
...
'set padding-left
Sub SetPaddingLeft(varPaddingLeft As String) As VTableOverflow
...
Sub AddComponent(child As String) As VTableOverflow
...
Sub SetID(varText As String) As VTableOverflow
	mname = varText
	Return Me
End Sub
...
public Sub AddToParent(targetID As String) As VTableOverflow
...
Sub AddToApp(vap As VueApp) As VTableOverflow
	appLink = vap
	data = vap.data	
...
'update the state
Sub SetData(prop as string, value as object) As VTableOverflow
	data.put(prop, value)
	Return Me
End Sub
...
'set an event
Sub SetVOn(event As String) As VTableOverflow
...
Sub SetCaption(varText As String) As VTableOverflow
	If mElement <> Null Then
...
private Sub AddAttr(varName As String, actProp As String) As VTableOverflow
...
public Sub SetClasses(Classes As String) As VTableOverflow
	If mElement <> Null Then
...
public Sub SetStyle(Style As String) As VTableOverflow
	If mElement <> Null Then
...
Sub AddClass(classNames As List) As VTableOverflow
...
Sub SetStyles(m As Map) As VTableOverflow
...
Sub SetAttrs(props As Map) As VTableOverflow
...
'set an attribute
Sub SetAttr(prop As String, value As String) As VTableOverflow
...
'bind an attribute
Sub SetVBind(prop As String, value As String) As VTableOverflow
...
'set a single style
Sub SetStyleSingle(prop As String, value As String) As VTableOverflow
	If BANano.IsUndefined(prop) or BANano.IsNull(prop) Then prop = ""
	If BANano.IsUndefined(value) or BANano.IsNull(value) Then value = ""
	if prop = "" then return me
...
Sub Build(props As Map, styleProps As Map, classNames As List, loose As List) As VTableOverflow
	If loose <> Null Then
...
'bind classes
Sub SetVClass(classObj as string) As VTableOverflow
	SetVBind("class", classObj)
	Return Me
End Sub

'bind styles
Sub SetVStyle(styleObj as string) As VTableOverflow
	SetVBind("style", styleObj)
	Return Me
End Sub

'set color
Sub SetColor1(varColor As String) As VTableOverflow
...
'set color intensity
Sub SetColorIntensity(varColor As String, varIntensity As String) As VTableOverflow
...
'set text color
Sub SetTextColor1(varColor As String) As VTableOverflow
...
Sub SetTextColorIntensity(varColor As String, varIntensity As String) As VTableOverflow
	Dim sColor As String = $"${varColor}--text"$
...
'set direct method
Sub SetMethod(Module As Object,methodName As String) As VTableOverflow
	methodName = methodName.ToLowerCase
...
'toggle
Sub Toggle As VTableOverflow
	If sVShow = "" Then
		Log($"VTableOverflow.Toggle - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
'hide
Sub Hide As VTableOverflow
	If sVShow = "" Then
		Log($"VTableOverflow.Hide - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
'show
Sub Show As VTableOverflow
	If sVShow = "" Then
		Log($"VTableOverflow.Show - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
Sub SetClassOnOff(clsName as string, clsValue As Boolean) As VTableOverflow
	if svBindClass = "" then
		Log($"VTableOverflow.VBindClass - the v-bind:class for ${mName} has not been set!"$)
		Return Me
	end if
...
'set style 
Sub SetStyleOnOff(styleName as string, styleValue As Boolean) As VTableOverflow
	if svBindStyle = "" then
		Log($"VTableOverflow.VBindCStyle - the v-bind:style for ${mName} has not been set!"$)
		Return Me
	end if
...
'required
'Sub SetRequiredOnOff(b As Boolean) As VTableOverflow
'	If sRequired = "" Then
'		Log($"VTableOverflow.Required - the required for ${mName} has not been set!"$)
'		Return Me
'	End If
...
'read only
'Sub SetReadOnlyOnOff(b As Boolean) As VTableOverflow
'	If sReadonly = "" Then
'		Log($"VTableOverflow.ReadOnly - the readonly for ${mName} has not been set!"$)
'		Return Me
'	End If
...
'disabled
'Sub SetDisabledOnOff(b As Boolean) As VTableOverflow
'	If sDisabled = "" Then
'		Log($"VTableOverflow.Disabled - the disabled for ${mName} has not been set!"$)
'		Return Me
'	End If
