...
Sub SetOnClick1() As VRouterView
...
'update the state
Sub SetData(prop as string, value as object) As VRouterView
	data.put(prop, value)
	Return Me
End Sub
...
'toggle
Sub Toggle As VRouterView
	If sVShow = "" Then
		Log($"VRouterView.Toggle - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
'hide
Sub Hide As VRouterView
	If sVShow = "" Then
		Log($"VRouterView.Hide - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
'show
Sub Show As VRouterView
	If sVShow = "" Then
		Log($"VRouterView.Show - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
Sub SetClassOnOff(clsName as string, clsValue As Boolean) As VRouterView
	if svBindClass = "" then
		Log($"VRouterView.VBindClass - the v-bind:class for ${mName} has not been set!"$)
		Return Me
	end if
...
'set style 
Sub SetStyleOnOff(styleName as string, styleValue As Boolean) As VRouterView
	if svBindStyle = "" then
		Log($"VRouterView.VBindCStyle - the v-bind:style for ${mName} has not been set!"$)
		Return Me
	end if
...
'required
Sub SetRequiredOnOff(b As Boolean) As VRouterView
	If sRequired = "" Then
		Log($"VRouterView.Required - the required for ${mName} has not been set!"$)
		Return Me
	End If
...
'read only
Sub SetReadOnlyOnOff(b As Boolean) As VRouterView
	If sReadonly = "" Then
		Log($"VRouterView.ReadOnly - the readonly for ${mName} has not been set!"$)
		Return Me
	End If
...
'disabled
Sub SetDisabledOnOff(b As Boolean) As VRouterView
	If sDisabled = "" Then
		Log($"VRouterView.Disabled - the disabled for ${mName} has not been set!"$)
		Return Me
	End If
