...
Version=8.3
@EndOfDesignText@
'Custom BANano View class: VContent
#IgnoreWarnings:12
...
Private mTagName As String = "v-main" 
...
Public Sub Initialize (CallBack As Object, Name As String, EventName As String) As VContent 
mName = Name 
...
'set disabled
Sub SetDisabled(varDisabled As String) As VContent
...
'set key
Sub SetKey(varKey As String) As VContent
sKey = varKey
SetAttr("key", sKey)
Return Me
End Sub
...
'set parent-id
Sub SetParentId(varParentId As String) As VContent
...
'set readonly
Sub SetReadonly(varReadonly As String) As VContent
...
'set ref
Sub SetRef(varRef As String) As VContent
sRef = varRef
SetAttr("ref", sRef)
Return Me
End Sub
...
'set required
Sub SetRequired(varRequired As String) As VContent
...
'set tag
Sub SetTag(varTag As String) As VContent
sTag = varTag
SetAttr("tag", sTag)
Return Me
End Sub
...
'set v-bind:class
Sub SetVBindClass(varVBindClass As String) As VContent
...
'set v-bind:style
Sub SetVBindStyle(varVBindStyle As String) As VContent
...
'set v-cloak
Sub SetVCloak(varVCloak As Boolean) As VContent
bVCloak = varVCloak
...
'set v-else
Sub SetVElse(varVElse As String) As VContent
sVElse = varVElse
...
'set v-else-if
Sub SetVElseIf(varVElseIf As String) As VContent
sVElseIf = varVElseIf
...
'set v-for
Sub SetVFor(varVFor As String) As VContent
sVFor = varVFor
SetAttr("v-for", sVFor)
Return Me
End Sub
...
'set v-html
Sub SetVHtml(varVHtml As String) As VContent
sVHtml = varVHtml
...
'set v-if
Sub SetVIf(varVIf As String) As VContent
sVIf = varVIf
SetAttr("v-if", sVIf)
Return Me
End Sub
...
'set v-model
Sub SetVModel(varVModel As String) As VContent
sVModel = varVModel
...
'set v-once
Sub SetVOnce(varVOnce As Boolean) As VContent
bVOnce = varVOnce
...
'set v-pre
Sub SetVPre(varVPre As Boolean) As VContent
bVPre = varVPre
SetAttr("v-pre", bVPre)
Return Me
End Sub
...
'set v-show
Sub SetVShow(varVShow As String) As VContent
sVShow = varVShow
...
'set v-text
Sub SetVText(varVText As String) As VContent
sVText = varVText
...
'set border-color
Sub SetBorderColor(varBorderColor As String) As VContent
...
'set border-style
Sub SetBorderStyle(varBorderStyle As String) As VContent
...
'set border-width
Sub SetBorderWidth(varBorderWidth As String) As VContent
...
'set border-radius
Sub SetBorderRadius(varBorderRadius As String) As VContent
...
'set margin-top
Sub SetMarginTop(varMarginTop As String) As VContent
...
'set margin-right
Sub SetMarginRight(varMarginRight As String) As VContent
...
'set margin-bottom
Sub SetMarginBottom(varMarginBottom As String) As VContent
...
'set margin-left
Sub SetMarginLeft(varMarginLeft As String) As VContent
...
'set padding-top
Sub SetPaddingTop(varPaddingTop As String) As VContent
...
'set padding-right
Sub SetPaddingRight(varPaddingRight As String) As VContent
...
'set padding-bottom
Sub SetPaddingBottom(varPaddingBottom As String) As VContent
...
'set padding-left
Sub SetPaddingLeft(varPaddingLeft As String) As VContent
...
Sub AddComponent(child As String) As VContent
...
Sub SetID(varText As String) As VContent
	mname = varText
	Return Me
End Sub
...
public Sub AddToParent(targetID As String) As VContent
...
Sub AddToApp(vap As VueApp) As VContent
	appLink = vap
	data = vap.data	
...
'update the state
Sub SetData(prop as string, value as object) As VContent
	data.put(prop, value)
	Return Me
End Sub
...
'set an event
Sub SetVOn(event As String) As VContent
...
Sub SetCaption(varText As String) As VContent
	If mElement <> Null Then
...
private Sub AddAttr(varName As String, actProp As String) As VContent
...
public Sub SetClasses(Classes As String) As VContent
	If mElement <> Null Then
...
public Sub SetStyle(Style As String) As VContent
	If mElement <> Null Then
...
Sub AddClass(classNames As List) As VContent
...
Sub SetStyles(m As Map) As VContent
...
Sub SetAttrs(props As Map) As VContent
...
'set an attribute
Sub SetAttr(prop As String, value As String) As VContent
...
'bind an attribute
Sub SetVBind(prop As String, value As String) As VContent
...
'set a single style
Sub SetStyleSingle(prop As String, value As String) As VContent
	If BANano.IsUndefined(prop) or BANano.IsNull(prop) Then prop = ""
	If BANano.IsUndefined(value) or BANano.IsNull(value) Then value = ""
	if prop = "" then return me
...
Sub Build(props As Map, styleProps As Map, classNames As List, loose As List) As VContent
	If loose <> Null Then
...
'bind classes
Sub SetVClass(classObj as string) As VContent
	SetVBind("class", classObj)
	Return Me
End Sub

'bind styles
Sub SetVStyle(styleObj as string) As VContent
	SetVBind("style", styleObj)
	Return Me
End Sub

'set color
Sub SetColor1(varColor As String) As VContent
...
'set color intensity
Sub SetColorIntensity(varColor As String, varIntensity As String) As VContent
...
'set text color
Sub SetTextColor1(varColor As String) As VContent
...
Sub SetTextColorIntensity(varColor As String, varIntensity As String) As VContent
	Dim sColor As String = $"${varColor}--text"$
...
'set direct method
Sub SetMethod(Module As Object,methodName As String) As VContent
	methodName = methodName.ToLowerCase
...
'toggle
Sub Toggle As VContent
	If sVShow = "" Then
		Log($"VContent.Toggle - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
'hide
Sub Hide As VContent
	If sVShow = "" Then
		Log($"VContent.Hide - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
'show
Sub Show As VContent
	If sVShow = "" Then
		Log($"VContent.Show - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
Sub SetClassOnOff(clsName as string, clsValue As Boolean) As VContent
	if svBindClass = "" then
		Log($"VContent.VBindClass - the v-bind:class for ${mName} has not been set!"$)
		Return Me
	end if
...
'set style 
Sub SetStyleOnOff(styleName as string, styleValue As Boolean) As VContent
	if svBindStyle = "" then
		Log($"VContent.VBindCStyle - the v-bind:style for ${mName} has not been set!"$)
		Return Me
	end if
...
'required
'Sub SetRequiredOnOff(b As Boolean) As VContent
'	If sRequired = "" Then
'		Log($"VContent.Required - the required for ${mName} has not been set!"$)
'		Return Me
'	End If
...
'read only
'Sub SetReadOnlyOnOff(b As Boolean) As VContent
'	If sReadonly = "" Then
'		Log($"VContent.ReadOnly - the readonly for ${mName} has not been set!"$)
'		Return Me
'	End If
...
'disabled
'Sub SetDisabledOnOff(b As Boolean) As VContent
'	If sDisabled = "" Then
'		Log($"VContent.Disabled - the disabled for ${mName} has not been set!"$)
'		Return Me
'	End If
