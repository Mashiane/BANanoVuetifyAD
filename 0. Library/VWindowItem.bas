2020-07-08 02:40:11 B4J=true
Group=Default Group
ModulesStructureVersion=1
Type=Class
Version=8.3
@EndOfDesignText@
'Custom BANano View class: VWindowItem
#IgnoreWarnings:12
...
Private mTagName As String = "v-window-item" 
...
Public Sub Initialize (CallBack As Object, Name As String, EventName As String) As VWindowItem 
mName = Name 
...
'set active-class
Sub SetActiveClass(varActiveClass As String) As VWindowItem
...
'set disabled
Sub SetDisabled(varDisabled As Boolean) As VWindowItem
...
'set eager
Sub SetEager(varEager As Boolean) As VWindowItem
bEager = varEager
...
'set key
Sub SetKey(varKey As String) As VWindowItem
sKey = varKey
SetAttr("key", sKey)
Return Me
End Sub
...
'set parent-id
Sub SetParentId(varParentId As String) As VWindowItem
...
'set readonly
Sub SetReadonly(varReadonly As String) As VWindowItem
...
'set ref
Sub SetRef(varRef As String) As VWindowItem
sRef = varRef
SetAttr("ref", sRef)
Return Me
End Sub
...
'set required
Sub SetRequired(varRequired As String) As VWindowItem
...
Sub SetReverseTransition(varReverseTransition As String) As VWindowItem
...
'set transition
Sub SetTransition(varTransition As String) As VWindowItem
...
'set v-bind:class
Sub SetVBindClass(varVBindClass As String) As VWindowItem
...
'set v-bind:style
Sub SetVBindStyle(varVBindStyle As String) As VWindowItem
...
'set v-cloak
Sub SetVCloak(varVCloak As Boolean) As VWindowItem
bVCloak = varVCloak
...
'set v-else
Sub SetVElse(varVElse As String) As VWindowItem
sVElse = varVElse
...
'set v-else-if
Sub SetVElseIf(varVElseIf As String) As VWindowItem
sVElseIf = varVElseIf
...
'set v-for
Sub SetVFor(varVFor As String) As VWindowItem
sVFor = varVFor
SetAttr("v-for", sVFor)
Return Me
End Sub
...
'set v-html
Sub SetVHtml(varVHtml As String) As VWindowItem
sVHtml = varVHtml
...
'set v-if
Sub SetVIf(varVIf As String) As VWindowItem
sVIf = varVIf
SetAttr("v-if", sVIf)
Return Me
End Sub
...
'set v-model
Sub SetVModel(varVModel As String) As VWindowItem
sVModel = varVModel
...
'set v-once
Sub SetVOnce(varVOnce As Boolean) As VWindowItem
bVOnce = varVOnce
...
'set v-pre
Sub SetVPre(varVPre As Boolean) As VWindowItem
bVPre = varVPre
SetAttr("v-pre", bVPre)
Return Me
End Sub
...
'set v-show
Sub SetVShow(varVShow As String) As VWindowItem
sVShow = varVShow
...
'set v-text
Sub SetVText(varVText As String) As VWindowItem
sVText = varVText
...
'set value
Sub SetValue(varValue As String) As VWindowItem
sValue = varValue
...
'set border-color
Sub SetBorderColor(varBorderColor As String) As VWindowItem
...
'set border-style
Sub SetBorderStyle(varBorderStyle As String) As VWindowItem
...
'set border-width
Sub SetBorderWidth(varBorderWidth As String) As VWindowItem
...
'set border-radius
Sub SetBorderRadius(varBorderRadius As String) As VWindowItem
...
'set margin-top
Sub SetMarginTop(varMarginTop As String) As VWindowItem
...
'set margin-right
Sub SetMarginRight(varMarginRight As String) As VWindowItem
...
'set margin-bottom
Sub SetMarginBottom(varMarginBottom As String) As VWindowItem
...
'set margin-left
Sub SetMarginLeft(varMarginLeft As String) As VWindowItem
...
'set padding-top
Sub SetPaddingTop(varPaddingTop As String) As VWindowItem
...
'set padding-right
Sub SetPaddingRight(varPaddingRight As String) As VWindowItem
...
'set padding-bottom
Sub SetPaddingBottom(varPaddingBottom As String) As VWindowItem
...
'set padding-left
Sub SetPaddingLeft(varPaddingLeft As String) As VWindowItem
...
Sub AddComponent(child As String) As VWindowItem
...
Sub SetID(varText As String) As VWindowItem
	mname = varText
	Return Me
End Sub
...
public Sub AddToParent(targetID As String) As VWindowItem
...
Sub AddToApp(vap As VueApp) As VWindowItem
	appLink = vap
	data = vap.data	
...
'update the state
Sub SetData(prop as string, value as object) As VWindowItem
	data.put(prop, value)
	Return Me
End Sub
...
'set an event
Sub SetVOn(event As String) As VWindowItem
...
Sub SetCaption(varText As String) As VWindowItem
	If mElement <> Null Then
...
private Sub AddAttr(varName As String, actProp As String) As VWindowItem
...
public Sub SetClasses(Classes As String) As VWindowItem
	If mElement <> Null Then
...
public Sub SetStyle(Style As String) As VWindowItem
	If mElement <> Null Then
...
Sub AddClass(classNames As List) As VWindowItem
...
Sub SetStyles(m As Map) As VWindowItem
...
Sub SetAttrs(props As Map) As VWindowItem
...
'set an attribute
Sub SetAttr(prop As String, value As String) As VWindowItem
...
'bind an attribute
Sub SetVBind(prop As String, value As String) As VWindowItem
...
'set a single style
Sub SetStyleSingle(prop As String, value As String) As VWindowItem
	If BANano.IsUndefined(prop) or BANano.IsNull(prop) Then prop = ""
	If BANano.IsUndefined(value) or BANano.IsNull(value) Then value = ""
	if prop = "" then return me
...
Sub Build(props As Map, styleProps As Map, classNames As List, loose As List) As VWindowItem
	If loose <> Null Then
...
'bind classes
Sub SetVClass(classObj as string) As VWindowItem
	SetVBind("class", classObj)
	Return Me
End Sub

'bind styles
Sub SetVStyle(styleObj as string) As VWindowItem
	SetVBind("style", styleObj)
	Return Me
End Sub

'set color
Sub SetColor1(varColor As String) As VWindowItem
...
'set color intensity
Sub SetColorIntensity(varColor As String, varIntensity As String) As VWindowItem
...
'set text color
Sub SetTextColor1(varColor As String) As VWindowItem
...
Sub SetTextColorIntensity(varColor As String, varIntensity As String) As VWindowItem
	Dim sColor As String = $"${varColor}--text"$
...
'set direct method
Sub SetMethod(Module As Object,methodName As String) As VWindowItem
	methodName = methodName.ToLowerCase
...
'toggle
Sub Toggle As VWindowItem
	If sVShow = "" Then
		Log($"VWindowItem.Toggle - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
'hide
Sub Hide As VWindowItem
	If sVShow = "" Then
		Log($"VWindowItem.Hide - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
'show
Sub Show As VWindowItem
	If sVShow = "" Then
		Log($"VWindowItem.Show - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
Sub SetClassOnOff(clsName as string, clsValue As Boolean) As VWindowItem
	if svBindClass = "" then
		Log($"VWindowItem.VBindClass - the v-bind:class for ${mName} has not been set!"$)
		Return Me
	end if
...
'set style 
Sub SetStyleOnOff(styleName as string, styleValue As Boolean) As VWindowItem
	if svBindStyle = "" then
		Log($"VWindowItem.VBindCStyle - the v-bind:style for ${mName} has not been set!"$)
		Return Me
	end if
...
'required
'Sub SetRequiredOnOff(b As Boolean) As VWindowItem
'	If sRequired = "" Then
'		Log($"VWindowItem.Required - the required for ${mName} has not been set!"$)
'		Return Me
'	End If
...
'read only
'Sub SetReadOnlyOnOff(b As Boolean) As VWindowItem
'	If sReadonly = "" Then
'		Log($"VWindowItem.ReadOnly - the readonly for ${mName} has not been set!"$)
'		Return Me
'	End If
...
'disabled
'Sub SetDisabledOnOff(b As Boolean) As VWindowItem
'	If sDisabled = "" Then
'		Log($"VWindowItem.Disabled - the disabled for ${mName} has not been set!"$)
'		Return Me
'	End If
