...
#DesignerProperty: Key: Alt, DisplayName: Alt, FieldType: String, DefaultValue: , Description: Set alt
#DesignerProperty: Key: AspectRatio, DisplayName: AspectRatio, FieldType: String, DefaultValue: , Description: Set aspect-ratio
#DesignerProperty: Key: Contain, DisplayName: Contain, FieldType: Boolean, DefaultValue: false , Description: Set contain
#DesignerProperty: Key: Eager, DisplayName: Eager, FieldType: Boolean, DefaultValue: false , Description: Set eager
#DesignerProperty: Key: Gradient, DisplayName: Gradient, FieldType: String, DefaultValue: , Description: Set gradient
#DesignerProperty: Key: Height, DisplayName: Height, FieldType: String, DefaultValue: , Description: Set height
#DesignerProperty: Key: LazySrc, DisplayName: LazySrc, FieldType: String, DefaultValue: , Description: Set lazy-src
...
#DesignerProperty: Key: Position, DisplayName: Position, FieldType: String, DefaultValue: , Description: Set position
#DesignerProperty: Key: Sizes, DisplayName: Sizes, FieldType: String, DefaultValue: , Description: Set sizes
#DesignerProperty: Key: Srcset, DisplayName: Srcset, FieldType: String, DefaultValue: , Description: Set srcset
...
Sub Class_Globals
	...
	Public sAspectRatio As String = ""
	Public bContain As Boolean = false
	Public bEager As Boolean = false
	Public sGradient As String = ""
	Public sHeight As String = ""
	Public sLazySrc As String = ""
	Public sMaxHeight As String = ""
	Public sMaxWidth As String = ""
	Public sMinHeight As String = ""
	Public sMinWidth As String = ""
	Public sPosition As String = "center center"
	Public sSizes As String = ""
	Public sSrcset As String = ""
	Public sTransition As String = "fade-transition"
...
Public Sub DesignerCreateView (Target As BANanoElement, props As Map)
	...
	If props <> Null Then
		...
		AddAttr(Attributes, sAlt, "s", "alt")
		AddAttr(Attributes, sAspectRatio, "s", "aspect-ratio")
		AddAttr(Attributes, bContain, "b", "contain")
		AddAttr(Attributes, bEager, "b", "eager")
		AddAttr(Attributes, sGradient, "s", "gradient")
		AddAttr(Attributes, sHeight, "s", "height")
		AddAttr(Attributes, sLazySrc, "s", "lazy-src")
		...
		AddAttr(Attributes, sPosition, "s", "position")
		AddAttr(Attributes, sSizes, "s", "sizes")
		AddAttr(Attributes, sSrcset, "s", "srcset")
...
Sub SetAlt(varAlt As String)
	sAlt = varAlt
	mElement.SetAttr("alt", sAlt)
End Sub

'set aspect-ratio
Sub SetAspectRatio(varAspectRatio As String)
	sAspectRatio = varAspectRatio
	mElement.SetAttr("aspect-ratio", sAspectRatio)
End Sub

'set contain
Sub SetContain(varContain As Boolean)
	bContain = varContain
	mElement.SetAttr("contain", bContain)
End Sub
...
'set gradient
Sub SetGradient(varGradient As String)
	sGradient = varGradient
	mElement.SetAttr("gradient", sGradient)
End Sub
...
'set lazy-src
Sub SetLazySrc(varLazySrc As String)
	sLazySrc = varLazySrc
	mElement.SetAttr("lazy-src", sLazySrc)
End Sub
...
'set options
Sub SetOptions(varOptions As Map)
	mElement.SetAttr("options", varOptions)
End Sub

'set position
Sub SetPosition(varPosition As String)
	sPosition = varPosition
	mElement.SetAttr("position", sPosition)
End Sub

'set sizes
Sub SetSizes(varSizes As String)
	sSizes = varSizes
	mElement.SetAttr("sizes", sSizes)
End Sub
...
'set srcset
Sub SetSrcset(varSrcset As String)
	sSrcset = varSrcset
	mElement.SetAttr("srcset", sSrcset)
End Sub
...
'set on error event
Sub SetOnError
	Dim mName As String = $"${mEventName}_error"$
	mName = mName.tolowercase
	If SubExists(mCallBack, mName) = False Then Return
	SetAttr("v-on:error", mName)
...
'set on load event
Sub SetOnLoad
	Dim mName As String = $"${mEventName}_load"$
	mName = mName.tolowercase
	If SubExists(mCallBack, mName) = False Then Return
	SetAttr("v-on:load", mName)
	Dim argument As Object 'ignore
	Dim cb As BANanoObject = BANano.CallBack(mCallBack, mName, Array(argument))
	methods.Put(mName, cb)
End Sub
