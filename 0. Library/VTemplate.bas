...
Version=8.3
@EndOfDesignText@
'Custom BANano View class: VTemplate
#IgnoreWarnings:12
...
Private mTagName As String = "template" 
...
Public Sub Initialize (CallBack As Object, Name As String, EventName As String) As VTemplate 
mName = Name 
...
'set disabled
Sub SetDisabled(varDisabled As String) As VTemplate
...
'set readonly
Sub SetReadonly(varReadonly As String) As VTemplate
...
'set ref
Sub SetRef(varRef As String) As VTemplate
sRef = varRef
SetAttr("ref", sRef)
Return Me
End Sub
...
'set required
Sub SetRequired(varRequired As String) As VTemplate
...
'set v-bind:class
Sub SetVBindClass(varVBindClass As String) As VTemplate
...
'set v-bind:style
Sub SetVBindStyle(varVBindStyle As String) As VTemplate
...
'set v-cloak
Sub SetVCloak(varVCloak As Boolean) As VTemplate
bVCloak = varVCloak
...
'set v-else
Sub SetVElse(varVElse As String) As VTemplate
sVElse = varVElse
...
'set v-else-if
Sub SetVElseIf(varVElseIf As String) As VTemplate
sVElseIf = varVElseIf
...
'set v-for
Sub SetVFor(varVFor As String) As VTemplate
sVFor = varVFor
SetAttr("v-for", sVFor)
Return Me
End Sub
...
'set v-html
Sub SetVHtml(varVHtml As String) As VTemplate
sVHtml = varVHtml
...
'set v-if
Sub SetVIf(varVIf As String) As VTemplate
sVIf = varVIf
SetAttr("v-if", sVIf)
Return Me
End Sub
...
'set v-model
Sub SetVModel(varVModel As String) As VTemplate
sVModel = varVModel
...
'set v-once
Sub SetVOnce(varVOnce As Boolean) As VTemplate
bVOnce = varVOnce
...
'set v-pre
Sub SetVPre(varVPre As Boolean) As VTemplate
bVPre = varVPre
SetAttr("v-pre", bVPre)
Return Me
End Sub
...
'set v-show
Sub SetVShow(varVShow As String) As VTemplate
sVShow = varVShow
...
'set v-text
Sub SetVText(varVText As String) As VTemplate
sVText = varVText
...
'set border-color
Sub SetBorderColor(varBorderColor As String) As VTemplate
...
'set border-style
Sub SetBorderStyle(varBorderStyle As String) As VTemplate
...
'set border-width
Sub SetBorderWidth(varBorderWidth As String) As VTemplate
...
'set border-radius
Sub SetBorderRadius(varBorderRadius As String) As VTemplate
...
'set margin-top
Sub SetMarginTop(varMarginTop As String) As VTemplate
...
'set margin-right
Sub SetMarginRight(varMarginRight As String) As VTemplate
...
'set margin-bottom
Sub SetMarginBottom(varMarginBottom As String) As VTemplate
...
'set margin-left
Sub SetMarginLeft(varMarginLeft As String) As VTemplate
...
'set padding-top
Sub SetPaddingTop(varPaddingTop As String) As VTemplate
...
'set padding-right
Sub SetPaddingRight(varPaddingRight As String) As VTemplate
...
'set padding-bottom
Sub SetPaddingBottom(varPaddingBottom As String) As VTemplate
...
'set padding-left
Sub SetPaddingLeft(varPaddingLeft As String) As VTemplate
...
public Sub AddToParent(targetID As String) As VTemplate
...
Sub AddToApp(vap As VueApp) As VTemplate
	appLink = vap
	data = vap.data	
...
'set an event
Sub SetVOn(event As String) As VTemplate
...
Sub SetCaption(varText As String) As VTemplate
	If mElement <> Null Then
...
public Sub SetClasses(Classes As String) As VTemplate
	If mElement <> Null Then
...
public Sub SetStyle(Style As String) As VTemplate
	If mElement <> Null Then
...
Sub AddClass(classNames As List) As VTemplate
...
Sub SetStyles(m As Map) As VTemplate
...
Sub SetAttrs(props As Map) As VTemplate
...
'set an attribute
Sub SetAttr(prop As String, value As String) As VTemplate
...
'bind an attribute
Sub SetVBind(prop As String, value As String) As VTemplate
...
'set a single style
Sub SetStyleSingle(prop As String, value As String) As VTemplate
	If BANano.IsUndefined(prop) or BANano.IsNull(prop) Then prop = ""
	If BANano.IsUndefined(value) or BANano.IsNull(value) Then value = ""
	if prop = "" then return me
...
Sub Build(props As Map, styleProps As Map, classNames As List, loose As List) As VTemplate
	If loose <> Null Then
...
'bind classes
Sub SetVClass(classObj as string) As VTemplate
	SetVBind("class", classObj)
	Return Me
End Sub

'bind styles
Sub SetVStyle(styleObj as string) As VTemplate
	SetVBind("style", styleObj)
	Return Me
End Sub

'set color
Sub SetColor1(varColor As String) As VTemplate
...
'set color intensity
Sub SetColorIntensity(varColor As String, varIntensity As String) As VTemplate
...
'set text color
Sub SetTextColor1(varColor As String) As VTemplate
...
Sub SetTextColorIntensity(varColor As String, varIntensity As String) As VTemplate
	Dim sColor As String = $"${varColor}--text"$
...
'set direct method
Sub SetMethod(Module As Object,methodName As String) As VTemplate
	methodName = methodName.ToLowerCase
...
'toggle
Sub Toggle As VTemplate
	If sVShow = "" Then
		Log($"VTemplate.Toggle - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
'hide
Sub Hide As VTemplate
	If sVShow = "" Then
		Log($"VTemplate.Hide - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
'show
Sub Show As VTemplate
	If sVShow = "" Then
		Log($"VTemplate.Show - the v-show for ${mName} has not been set!"$)
		Return Me
	End If
...
Sub SetClassOnOff(clsName as string, clsValue As Boolean) As VTemplate
	if svBindClass = "" then
		Log($"VTemplate.VBindClass - the v-bind:class for ${mName} has not been set!"$)
		Return Me
	end if
...
'set style 
Sub SetStyleOnOff(styleName as string, styleValue As Boolean) As VTemplate
	if svBindStyle = "" then
		Log($"VTemplate.VBindCStyle - the v-bind:style for ${mName} has not been set!"$)
		Return Me
	end if
